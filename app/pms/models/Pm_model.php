...
class Pm_model extends CI_Model{
	...
	public function limit_page($uid, $page) {
		...
		if ($members) {
			...
			foreach ($members as $key => $data) {
				...
				$data['pmid'] = $data['plid'];
				$lastmessage = string2array($data['lastmessage']);
	...
	public function read_limit_page($touid, $page) {
		...
		$plist = $this->db->where('min_max', $min_max)->get('pm_lists')->row_array();
		if (!$plist) {
            return NULL;
        }
		...
		$this->db->query("UPDATE ".$this->db->dbprefix('pm_members')." SET isnew=0 WHERE plid=".intval($plid)." AND uid=".intval($uid)." AND isnew=1");
	...
	public function set_read($uid, $ids) {
		...
		foreach ($ids as $plid) {
			$this->db
				 ->query("UPDATE ".$this->db->dbprefix('pm_members')." SET isnew=0 WHERE plid=".intval($plid)." AND uid=".intval($uid)." AND isnew=1");
		}
	}
	...
	public function send($fromuid, $fromusername, $data) {
		...
		$subject = safe_replace(htmlspecialchars($data['subject']));
		$message = safe_replace(htmlspecialchars($data['message']));
		...
		$lastmessage = array2string($lastmessage);
