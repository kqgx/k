...
class Check extends M_Controller {
    ...
    private function _dir_write() {
		...
		$dir = array(
            DATAPATH => '无法生成系统缓存文件',
            CONFPATH.'' => '无法生成系统配置文件',
            CONFPATH.'domain.php' => '无法生成系统配置文件',
            CONFPATH.'rewrite.php' => '无法生成伪静态配置文件',
            CONFPATH.'site/' => '无法生成网站配置文件',
            CONFPATH.'site/1.php' => '无法生成网站配置文件',
            DATAPATH.'cache/' => '无法生成系统配置文件，会导致系统配置无效',
            DATAPATH.'views/' => '无法生成模板解析文件',
            DATAPATH.'session/' => '无法生成session会话文件',
            DATAPATH.'cron/' => '无法执行任务计划',
            DATAPATH.'index/' => '无法生成首页静态页面',
            DATAPATH.'file/' => '无法存储文件缓存内容',
            DATAPATH.'log/error/' => '无法保存错误日志',
            DATAPATH.'log/option/' => '无法存储操作日志',
			SYS_UPLOAD_PATH.'/' => '无法上传附件',
			MTBASE.'file/thumb/' => '无法生成缩略图缓存文件',
		);
        ...
        if ($local) {
            foreach ($local as $mdir) {
                if (is_file(WEBPATH.'module/'.$mdir.'/config/module.php')) {
                    $dir[WEBPATH.'module/'.$mdir.'/config/'] = '无法生成模块【'.$dir.'】配置文件';
                }
            }
        }
    ...
    private function _web_template() {
        ...
        if (!is_file(VIEWPATH.'pc/web/'.SITE_TEMPLATE.'/common/index.html')) {
            return $this->halt('网站前端模板【电脑版】不存在：/pc/web/'.SITE_TEMPLATE.'/common/index.html', 0);
        } elseif (!is_file(VIEWPATH.'pc/member/'.SITE_TEMPLATE.'/common/index.html')) {
            return  $this->halt('会员中心模板【电脑版】不存在：/pc/member/'.SITE_TEMPLATE.'/common/index.html', 0);
        } elseif (!is_file(VIEWPATH.'mobile/web/'.SITE_TEMPLATE.'/common/index.html')) {
            return $this->halt('网站前端模板【手机版】不存在：/mobile/web/'.SITE_TEMPLATE.'/common/index.html', 0);
        } elseif (!is_file(VIEWPATH.'mobile/member/'.SITE_TEMPLATE.'/common/index.html')) {
    ...
    private function _update() {
        ...
        if ($module) {
            foreach ($module as $m) {
                ...
                foreach ($syn as $t) {
                    !is_file(WEBPATH.'module/'.$dir.'/controllers'.$t) && @copy(DATAPATH.'update/module'.$t, WEBPATH.'module/'.$dir.'/controllers'.$t);
                }
                ...
                foreach ($this->site_info as $siteid => $t) {
                    if (isset($data['site'][$siteid]['use']) && $data['site'][$siteid]['use']) {
                        ...
                        if (!$this->db->query("SHOW TABLES LIKE '".$this->db->dbprefix.$siteid.'_'.$dirname."_comment'")->row_array()) {
                            $this->models('Module/comment')->module($dirname);
                            $this->models('Module/comment')->install_sql($siteid);
                            if ($data['extend']
                                && !$this->db->query("SHOW TABLES LIKE '".$this->db->dbprefix.$siteid.'_'.$dirname."_extend_comment'")->row_array()) {
                                $this->models('Module/comment')->extend($dirname);
                                $this->models('Module/comment')->install_sql($siteid);
                            }
                        }
                        // 模块回收表结构
