...
require_once MODELS.'Module/Extends_Module.php';
...
class Html extends D_Module {
    ...
    public function __construct() {
		...
		$this->template->assign('menu', $this->get_menu_v3(array(
		    L('生成静态') => array('admin/html/index', 'file'),
		)));
    }
    ...
    public function index() {
        ...
        if ($MOD) {
            foreach ($MOD as $t) {
                ...
                $t['select'] = $this->select_category_new($t['category'], 0, 'multiple name=\'data[catid][]\' style="min-width:200px;height:250px;"', L('-- 全部 --'), 'category');
                //$t['page_select'] = $page ? $this->select_category_new($page, 0, 'multiple name=\'data[catid][]\' style="min-width:200px;height:250px;"', L('-- 全部 --'), 'page') : '';
                $module[$t['dirname']] = $t;
            }
        }

        // 生成权限文件
        !dr_html_auth(1) && $this->_admin_msg(L('/data/views/ 无法写入文件'));
        ...
        $this->template->assign(array(
            'share' => $share ? $this->select_category_new($share, 0, 'multiple name=\'data[catid][]\' style="min-width:200px;height:250px;"', L('-- 全部 --'), 'category') : '',
            'module' => $module,
        ));
    ...
    public function tohtml() {
        ...
        if ($dir) {
            ...
            if (count($this->site_info) > 1 || strpos($mod['url'], 'index.php') !== false || $mod['domain']) {
                // 动态模式
                $file = DATAPATH.'index/'.(IS_MOBILE ? 'mobile-' : '').DOMAIN_NAME.'-'.$dir.'.html';
            } else {
                // 自定义url
                // 格式化生成文件
                $file = dr_format_html_file($this->_remove_domain($mod['url']));
            }
            $html = dr_catcher_data(SITE_URL.'index.php?s='.$dir.'&cache=2014');
            if (!$html) {
                $this->admin_msg('系统环境无法从url中获取内容，尝试进行系统体检');
            }
            $size = file_put_contents($file, $html);
            if (!$size) {
                $this->admin_msg('生成失败，检查'.DATAPATH.'/index目录是否可写');
            }
            $this->admin_msg('生成成功（'.dr_format_file_size($size).'）', '', 1);
        } else {
            $file = DATAPATH.'index/'.(IS_MOBILE ? 'mobile-' : '').DOMAIN_NAME.'-home.html';
            $html = dr_catcher_data(SITE_URL.'index.php?cache=2014');
            if (!$html) {
                $this->admin_msg('系统环境无法从url中获取内容，尝试进行系统体检');
            }
            $size = file_put_contents($file, $html);
