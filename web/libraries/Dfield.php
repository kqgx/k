...
class Dfield {
	...
	public function get($name) {
		...
		$class = 'F_'.$name;
		$file = FCPATH.'libraries/Field/'.$name.'.php';

		if (!is_file($file) && $this->module) {
			if (is_file(WEBPATH.'module/'.$this->module.'/libraries/Field/'.$name.'.php')) {
				$file = WEBPATH.'module/'.$this->module.'/libraries/Field/'.$name.'.php';
			} else {
				if (IS_ADMIN) {
	...
	public function type() {
		...
		if ($this->module) {
			$dir = WEBPATH.'module/'.$this->module.'/libraries/Field/';
			if ($fp = @opendir($dir)) {
				while (FALSE !== ($file = readdir($fp))) {
					if (is_file($dir.$file) && strpos($file, '.php') !== FALSE) {
						$name = substr($file, 0, -4);
...
<div class="form-group" id="dr_row_{name}">
	...
	public function member_field_select() {
		...
		$str.= '<option value="username"> '.L('会员名称').' </option>';
		$str.= '<option value="email"> '.L('会员邮箱').' </option>';
		$str.= '<option value="groupid"> '.L('会员组ID').' </option>';
		$str.= '<option value="levelid"> '.L('会员等级ID').' </option>';
		$str.= '<option value="name"> '.L('姓名').' </option>';
		$str.= '<option value="phone"> '.L('电话').' </option>';
	...
	public function field_type($name = NULL, $length = NULL) {
		...
		<div class="form-group">
			<label class="col-md-2 control-label">'.L('类型').'： </label>
			<div class="col-md-9">
				<label><select class="form-control" name="data[setting][option][fieldtype]" onChange="setlength()" id="type">
					'.$select.'
				</select></label>
				<span class="help-block">'.L('根据你的实际情况选择字段类型，如果你不懂MySQL数据库知识就不要填写此项').'</span>
			</div>
		</div>
		<div class="form-group">
			<label class="col-md-2 control-label">'.L('长度/值').'： </label>
			<div class="col-md-9">
				<label><input type="text" class="form-control" size="10" value="'.$length.'" name="data[setting][option][fieldlength]"></label>
				<span class="help-block">'.L('如果你不懂MySQL数据库知识就不要填写此项').'</span>
			</div>
		</div>';
