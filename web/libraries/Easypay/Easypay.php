...
class Easypay
{
    ...
    public function __construct(array $config = [])
    {
        spl_autoload_register(function ($class) {
            static $ns = 'Yansongda\Pay';
            $path = __DIR__ . '' . str_replace('\\', '/', substr($class, strlen($ns))) . '.php';
            if (strpos($class, $ns) === 0 && is_file($path)) {
                include_once $path;
            }
        });
    ...
    protected function createGateway($gateway)
    {
        ...
        $gateway = 'Yansongda\Pay\Gateways\\' . ucfirst($this->drivers) . '\\' . ucfirst($gateway) . 'Gateway';
