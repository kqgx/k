...
class Template {
    ...
    // 去除默认defult目录
    public $_p_template = SITE_TEMPLATE=='default'?'':SITE_TEMPLATE.'/';
    public $_m_template = MEMBER_TEMPLATE=='default'?'':MEMBER_TEMPLATE.'/';
    ...
    public function __construct() {
        ...
        $this->_root_array = array(
            'pc' => VIEWPATH.'pc/web/'. $this->_p_template,
            'mobile' => VIEWPATH.'mobile/web/'.$this->_p_template
        );
        // 默认会员项目模板目录
        $this->_mroot_array = array(
            'pc' => VIEWPATH.'pc/member/' . $this->_m_template,
            'mobile' => VIEWPATH.'mobile/member/' . $this->_m_template,
        );
        // 默认后台模板目录
        $this->_aroot = FCPATH.'views/admin/';
        // 模板缓存目录
        $this->_cache = DATAPATH . 'views/';
    ...
    private function _init_var() {
        // 当前项目模板目录
        if (IS_ADMIN) { // 后台
            $this->_dir = defined('MODEL_DIR') ? MODELS.MODEL_DIR.'/views/admin/' : APPPATH.'views/admin/';
        } elseif (IS_MEMBER) { // 会员
            ...
            if (APP_DIR != 'member') { // 模块和应用的会员目录
                $this->_module_root_array = array(
                    'pc' => VIEWPATH.'pc/member/'.$this->_p_template . APP_DIR.'/',
                    'mobile' => VIEWPATH.'mobile/member/'.$this->_p_template . APP_DIR.'/'
                );
                $this->_dir = $this->_module_root_array[$this->_tname];
            }
        } elseif (defined(APP_DIR) && is_dir(FCPATH.'app/'.APP_DIR.'/')) { // 应用
            $this->_root = $this->_root_array[$this->_tname];
            $this->_module_root_array = array(
                'pc' => VIEWPATH.'pc/web/'.$this->_p_template . APP_DIR.'/',
                'mobile' => VIEWPATH.'mobile/web/'.$this->_p_template . APP_DIR.'/'
            );
            $this->_dir = $this->_module_root_array[$this->_tname];
        } 
        // elseif (defined('MODELS_DIR')  && is_dir('MODELS_DIR')) {
        //     $this->_dir = MODELS.MODEL_DIR.'/views/';
        // } 
        else { // 首页前端页面
     ...
     * @return  void
     */
    public function display($_name = NULL, $_dir = NULL) {
        // 处理变量
    ...
    public function get_file_name($file, $dir = NULL, $include = FALSE) {
        !$file && $file = $this->ci->router->class.'_'.$this->ci->router->method.'.html';
        ...
        } elseif ($file == 'msg.html' && is_file(VIEWPATH.'pc/web/msg.html')) {
            return VIEWPATH.'pc/web/msg.html';
        }
        if (defined("CT_HTML_FILE")) {
            return FCPATH.'views/admin/tpl_404.html';
        }
        show_error('模板文件 ('.(SYS_DEBUG ? $error : str_replace(VIEWPATH, '/', $error)).') 不存在', 200, '模板解析错误');
    }
    ...
    public function admin() {
        $this->_dir = $this->_mdir = APPPATH.'views/admin/';
        $this->_mroot = $this->_root = FCPATH.'views/admin/'; // 默认后台模板目录
    }
    ...
    public function module($dir = '') {
        ...
        // 默认模板目录
        $this->_module_root_array = array(
            'pc' => VIEWPATH.'pc/web/'. $this->_p_template .$module.'/',
            'mobile' => VIEWPATH.'mobile/web/'. $this->_m_template .$module.'/',
        );
    ...
    public function load_view_file($name) {
        ...
        $cache_file = $this->_cache.str_replace(array(WEBPATH, '/', '\\', DIRECTORY_SEPARATOR), array('', '.', '.', '.'), md5($name)).($this->mobile ? '.mobile.' : '');
        ...
        if (!is_file($cache_file) || (is_file($cache_file) && is_file($name) && filemtime($cache_file) < filemtime($name))) {
            ...
            is_dir(DATAPATH . 'views') or mkdir(DATAPATH.'views');
            @file_put_contents($cache_file, $content, LOCK_EX) === FALSE && show_error('请将模板缓存目录（'.DATAPATH.'views/）权限设为777', 200, '无写入权限');
        }
    ...
    public function list_tag($_params) {
        ...
        $system = array(
            ...
            'return' => '', // 返回变量
            'sbpage' => '', // 不按默认分页
            'module' => (defined('MOD_DIR') && MOD_DIR) ? MOD_DIR : $this->ci->data->dirname, // 模块名称
            'modelid' => '', // 模型id
            'keyword' => '', // 关键字
            'urlrule' => '', // 自定义分页规则
            'pagesize' => '', // 自定义分页数量
        ...
        switch ($system['action']) {
            ...
            case 'content': // 模块文档内容
                ...
                if (!class_exists('Content_model')) {
                    $file = WEBPATH.'module/'.$module['dirname'].'/models/Content_model.php';
                    if (!is_file($file)) {
                ...
                $name = 'list-action-content-'.$dirname.'-'.md5(array2string($param).array2string($system)).'-'.$page;
            ...
            case 'table': // 表名查询
                ...
                if (!$tableinfo) {
                    $tableinfo = $this->ci->models('system')->cache(); // 表结构缓存
                }
                if (!$tableinfo) {
            ...
            case 'form': // 表单调用
                ...
                $tableinfo = $this->ci->get_cache('table');
                if (!$tableinfo) {
                    $tableinfo = $this->ci->models('system')->cache();
                }
                if (!$tableinfo) {
                    return $this->_return($system['return'], '表结构缓存不存在'); // 没有表结构缓存时返回空
                }
            ...
            default :
                return $this->_return($system['return'], L('参数错误'));
                break;
        }
    }
     ...
     * @return  bool
     */
    private function _include($name, $dir = NULL) {
    ...
    private function _get_pagination($url, $pagesize, $total) {
        ...
        if (!$this->pagination) {
            ...
            is_file(APPPATH.'config/'.$name) ? include APPPATH.'config/'.$name : include CONFPATH.$name;
    ...
    // list 返回
    private function _return($return, $data = NULL, $sql = NULL, $total = NULL, $pages = NULL, $pagesize = NULL) {
        ...
        // 错误信息格式化
        if($error){
            $debug = $error = IS_API ? $error :  '<div style="padding:10px;margin:10px;margin-top:20px;border:1px solid #ffbe7a;background:#fffced;color:red;font-weight:bold;">'.$error."</div>";
        }
