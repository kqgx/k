...
class F_Text extends A_Field {
    ...
    public function __construct() {
		parent::__construct();
		$this->name = IS_ADMIN ? L('单行文本') : ''; // 字段名称
	...
	public function option($option) {
        ...
        if (TEXT_UNIQUE) {
            ...
            <div class="form-group">
                <label class="col-md-2 control-label">'.L('验证重复').'：</label>
                <div class="col-md-9">
                	<input type="checkbox" name="data[setting][option][unique]" '.($option['unique'] ? 'checked' : '').' value="1"  data-on-text="'.L('开启').'" data-off-text="'.L('关闭').'" data-on-color="success" data-off-color="danger" class="make-switch" data-size="small">
					<span class="help-block">'.L('开启将会判断此字段的唯一性（只对主表有效）').'</span>
                </div>
            </div>
            ';
        }
		return '
            ...
            <div class="form-group">
                <label class="col-md-2 control-label">'.L('移动端自动宽度').'：</label>
                <div class="col-md-9">
					<div class="radio-list">
						<label class="radio-inline"><input type="radio" value="0" name="data[setting][option][is_mb_auto]" '.(!$option['is_mb_auto'] ? 'checked' : '').'> '.L('是').'</label>
						<label class="radio-inline"><input type="radio" value="1" name="data[setting][option][is_mb_auto]" '.($option['is_mb_auto'] ? 'checked' : '').'> '.L('否').'</label>
					</div>
                </div>
            </div>
            <div class="form-group">
                <label class="col-md-2 control-label">'.L('密码框模式').'：</label>
                <div class="col-md-9">
                	<input type="checkbox" name="data[setting][option][ispwd]" '.($option['ispwd'] ? 'checked' : '').' value="1"  data-on-text="'.L('开启').'" data-off-text="'.L('关闭').'" data-on-color="success" data-off-color="danger" class="make-switch" data-size="small">

					<span class="help-block">'.L('开启之后它将作为密码框来显示').'</span>
                </div>
            </div>
			'.$unique.'
			<div class="form-group">
