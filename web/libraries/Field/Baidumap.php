	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		$str = '<script type="text/javascript">
				function map_'.$name.'_mark() {
				    '.$city_value.'
					art.dialog.open("/member.php?c=api&m=baidumap&&width='.$width.'&height='.$height.'&name='.$name.'&level='.$level.'&value='.$value.'&city="+city, {
						title: "BaiduMap",
						opacity: 0.1,
						width:'.$width.',
						height:'.$height.',
						...
						var value = iframe.document.getElementById("'.$name.'").value,
							old = "'.$value.'";
							if (value == "") {
								$("#result_'.$name.'").html("<font color=green> '.L('标注成功').'（"+value+"）</font>");
							} else if (value != old) {
								$("#result_'.$name.'").html("<font color=blue> '.L('标注成功').'（"+value+"）</font>");
							} else {
								$("#result_'.$name.'").html("<font color=red> '.L('尚未标注').'（"+value+"）</font>");
								return true;
							}
							$("#dr_'.$name.'").val(value);
							return true;
				...
				<button type="button" name="'.$name.'_mark" onclick="map_'.$name.'_mark()" id="'.$name.'_mark" class="btn blue btn-sm"> <i class="fa fa-tint"></i> '.L('标注位置').'</button>
				<input name="data['.$name.']" id="dr_'.$name.'" type="hidden" value="'.$value.'" />
				<span id="result_'.$name.'">'.($value ? '<font color=green> '.L('标注成功').'（'.$value.'）</font>' : '').'</span>'.$tips;
