...
class F_Related extends A_Field {
	...
	public function option($option) {
		...
		return '<div class="form-group">
                    <label class="col-md-2 control-label">'.L('模块').'：</label>
                    <div class="col-md-9">
                    <label><select class="form-control" name="data[setting][option][module]">
					'.$_option.'
					</select></label>
					<span class="help-block">'.L('必须选择一个模块作为关联数据源').'</span>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-9">
                        <div class="form-control-static">'.L('此字段不能参与搜索条件筛选').'</div>
                    </div>
                </div>';
	}
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		function dr_add_related_'.$name.'() {
			art.dialog.open("/member.php?c=api&m=related&site='.SITE_ID.'&module='.$module.'&limit='.intval($cfg['option']['limit']).'", {
				title: "'.$cname.'",
				opacity: 0.1,
				width: 700,
				height: 400,
