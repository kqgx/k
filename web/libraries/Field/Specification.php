...
class F_Specification extends A_Field {
	...
	public function option($option) {
		...
		return '<div class="form-group">
				...
				<div class="col-md-9" style="color:blue"> <div class="form-control-static">'.L('此字段只能用于商城模块mall的模块字段').'</div></div>
			</div>
			';
	}
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		// 字段默认值
		$value = @strlen($value) ? string2array($value) : ($_POST['data'][$name]);
		...
		<label class="radio-inline"><input id="dr_box_spec_1" type="radio" onclick="dr_spec_use(1)" name="data['.$name.'][use]" value="1" '.($open ? 'checked=""' : '').'> '.L('开启').'</label>
		<label class="radio-inline"><input id="dr_box_spec_0" type="radio" onclick="dr_spec_use(0)" name="data['.$name.'][use]" value="0" '.(!$open ? 'checked=""' : '').'> '.L('关闭').'</label>
		</div>
		...
		<fieldset id="dr_spec_table" class="blue pad-10" style="'.(!$open ? 'display:none;' : '').'background-color: #F8F8F8;border: 1px solid #ECECEC;width:'.$width.(is_numeric($width) ? 'px' : '').'">
			<a class="add" style="margin-left:10px;margin-bottom:15px;" title="'.L('添加属性分组').'" href="javascript:dr_spec_add_group();"></a>
			<a class="del" style="margin-right:10px;margin-bottom:15px;float:right" title="'.L('删除所有自定义属性和分组').'" href="javascript:dr_spec_delete();"></a>
			<div class="picList myspec" id="dr_'.$name.'_result">
				'.$result.'
			</div>
			<div style="font-weight: bold;margin-bottom:-10px;clear:both;padding:10px;background: #ffffff;">'.L('定价').'</div>
			<div class="picList" id="dr_mall_sku" style="clear:both;padding:10px;background: #ffffff;">
				<span style="color:red">'.L('您需要选择所有的规格属性，才能组合成完整的规格信息。').'</span>
			</div>
		</fieldset>
		<style>
		.color-box {
		...
		function dr_spec_delete() {
		    $("#dr_'.$name.'_result").html("<div><img src=\"'.THEME_PATH.'admin/images/loading-mini.gif\" />&nbsp;'.L('数据加载中').'");
			$.ajax({
				type: "POST",
				dataType: "json",
				url: "/member.php?s='.APP_DIR.'&c=api&m=specification&nodiy=1",
		...
		function dr_load_mall_specification() {
			...
			$.ajax({
				type: "POST",
				dataType: "json",
				url: "/member.php?s='.APP_DIR.'&c=api&m=specification",
