...
class F_Property extends A_Field {
	...
	public function option($option) {
		...
		<div class="form-group dr_option" id="dr_option_0">
			<label class="col-md-2 control-label"><a href="javascript:;" onclick="dr_add_option()" style="color:blue">[+]</a>&nbsp;'.L('字段说明').'：</label>
			<div class="col-md-9"><div class="form-control-static">'.L('选择框与复选框类型的选项值以,分隔').'</div></div>
		</div>';
		if ($option) {
			foreach ($option as $i => $t) {
				$str.= '<div class="form-group dr_option" id="dr_option_'.$i.'" >';
				$str.= '<label class="col-md-2 control-label"><a href="javascript:;" onclick="dr_add_option()" style="color:blue">[+]</a>&nbsp;'.L('属性名称').'：</label>';
				$str.= '<div class="col-md-9"><label><input type="text" name="data[setting][option]['.$i.'][name]" value="'.$t['name'].'" class="form-control" /></label>';
				$str.= '<label>&nbsp;&nbsp;'.L('类型').'：</label><label><select class="form-control" name="data[setting][option]['.$i.'][type]">';
				$str.= '<option value="1" '.($t['type'] == 1 ? "selected" : "").'> - '.L('文本框').' - </option>';
				$str.= '<option value="2" '.($t['type'] == 2 ? "selected" : "").'> - '.L('选择框').' - </option>';
				$str.= '<option value="3" '.($t['type'] == 3 ? "selected" : "").'> - '.L('复选框').' - </option>';
				$str.= '</select></label>';
				$str.= '<label>&nbsp;&nbsp;'.L('默认值/选项值').'：</label><label><input type="text" name="data[setting][option]['.$i.'][value]" value="'.$t['value'].'" class="form-control"></label> <label><a onclick="$(\'#dr_option_'.$i.'\').remove()" href="javascript:;">'.L('删除').'</a></label>';
				$str.= '</div></div>';
			}
		}
		$str.= '
		...
		function dr_add_option() {
			...
			html+= "<label class=\"col-md-2 control-label\"><a href=\"javascript:;\" onclick=\"dr_add_option()\" style=\"color:blue\">[+]</a>&nbsp;'.L('属性名称').'：</label>";
			html+= "<div class=\"col-md-9\">";
			html+= "<label><input type=\"text\" name=\"data[setting][option]["+id+"][name]\" value=\"\" class=\"form-control\" /></label>";
			html+= "<label>&nbsp;&nbsp;'.L('类型').'：</label><label><select class=\"form-control\" name=\"data[setting][option]["+id+"][type]\">";
			html+= "<option value=\"1\"> - '.L('文本框').' - </option>";
			html+= "<option value=\"2\"> - '.L('选择框').' - </option>";
			html+= "<option value=\"3\"> - '.L('复选框').' - </option>";
			html+= "</select></label>";
			html+= "<label>&nbsp;&nbsp;'.L('默认值/选项值').'：</label><label><input type=\"text\" name=\"data[setting][option]["+id+"][value]\" class=\"form-control\"></label><label><a onclick=\"$(\'#dr_option_"+id+"\').remove()\" href=\"javascript:;\">'.L('删除').'</a></label>";
			html+= "</div>";
			html+= "</div>";
		...
		return $str.
        '<div class="form-group">
				<label class="col-md-2 control-label">'.L('友情提示').'：</label>
				<div class="col-md-9" style="color:blue"> <div class="form-control-static">'.L('此字段不能参与搜索条件筛选').'</div></div>
			</div>';
	}
	...
	public function insert_value($field) {
		...
		$this->ci->data[$field['ismain']][$field['fieldname']] = array2string($data);
	}
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		// 字段默认值
		$value = $value ? string2array($value) : array();
		// 禁止修改
		...
		if ($value) {
			foreach ($value as $t) {
                ...
                $str.= '<a href="javascript:;" onclick="$(\'#dr_items_'.$name.'_'.$i.'\').remove()">'.L('删除').'</a></li>';
                $i++;
			}
		}
		...
		function dr_add_property_'.$name.'() {
			...
			html+= "<a href=\"javascript:;\" onclick=\"$(\'#dr_items_'.$name.'_"+id+"\').remove()\">'.L('删除').'</a></li>";
