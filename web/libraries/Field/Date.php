...
class F_Date extends A_Field {
    ...
    public function __construct() {
		parent::__construct();
		$this->name = IS_ADMIN ? L('日期时间') : ''; // 字段名称
	...
	public function option($option) {
		...
		return '
			...
			<div class="form-group">
				<label class="col-md-2 control-label">'.L('显示格式').'：</label>
				<div class="col-md-9">
					<label><input type="text" class="form-control" size="30" name="data[setting][option][format]" value="'.$option['format'].'"></label>
					<span class="help-block">'.L('如果留空采用网站默认设置格式，Y表示年，m表示月，d表示日，H表示小时，i表示分，s表示秒，禁止中文').'</span>
				</div>
			</div>
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
        ...
        if (APP_DIR && $name == 'updatetime') {
            $str.= '<label><input name="no_time" type="checkbox" value="1" /> '.L('不更新').'</label>';
        }
		$str.= $tips;
