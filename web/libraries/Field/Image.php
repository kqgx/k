...
class F_Image extends A_Field {
	...
	public function option($option) {
		...
		return '
			...
			<div class="form-group">
				<label class="col-md-2 control-label">'.L('远程图片模式').'：</label>
				<div class="col-md-9">
			...
			<div class="form-group">
				<label class="col-md-2 control-label">'.L('保存目录').'：</label>
				<div class="col-md-9">
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		// 上传的URL
		$url = '/member.php?&c=api&m=ajax_upload&name='.$name.'&siteid='.SITE_ID.'&code='.str_replace('=', '', dr_authcode($cfg['option']['size'].'|'.$this->get_upload_path($cfg['option']['uploadpath']), 'ENCODE'));
		// 字段默认值
		$file_value = '';
		$value && $value = string2array($value);
		// 默认值输出
		$info = array();
