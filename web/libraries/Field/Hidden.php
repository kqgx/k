	...
	public function option($option) {
		...
		return '
            <div class="form-group">
                <label class="col-md-2 control-label">'.L('后台是否显示').'：</label>
                <div class="col-md-9">
					<div class="radio-list">
						<label class="radio-inline"><input type="radio" value="1" name="data[setting][option][is_admin_show]" '.($option['is_admin_show'] ? 'checked' : '').'> '.L('是').'</label>
						<label class="radio-inline"><input type="radio" value="0" name="data[setting][option][is_admin_show]" '.(!$option['is_admin_show'] ? 'checked' : '').'> '.L('否').'</label>
					</div>
                </div>
            </div>
			<div class="form-group">
