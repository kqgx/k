	...
	public function option($option, $field = NULL) {
		...
		return '
				<div class="form-group">
                    <label class="col-md-2 control-label">'.L('可用字段').'：</label>
                    <div class="col-md-9">
                    <label><select class="form-control" name="xx" id="fxx">'.$_field.'</select></label>
                    <span class="help-block">'.L('同一种随机颜色的字段表示在同一个分组').'</span>
                    </div>
                </div>
				<div class="form-group">
                    <label class="col-md-2 control-label">'.L('分组规则').'：</label>
                    <div class="col-md-9">
                    <textarea name="data[setting][option][value]" id="fvalue" style="height:120px;" class="form-control">'.$option['value'].'</textarea>
					<span class="help-block">'.L('不支持html标签，注意每个字段只能存在于一个分组中，否则会出错').'</span>
                    </div>
                </div>
                <div class="form-group">
