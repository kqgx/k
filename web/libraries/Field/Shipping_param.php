...
class F_Shipping_param extends A_Field {
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		// 字段默认值
		$value = @strlen($value) ? string2array($value) : '';
		// 当字段必填时，加入html5验证标签
		...
		$str = '<label>'.L('体积').'：</label><label><input size=5 class="form-control" type="text" name="data['.$name.'][m3]" id="dr_'.$name.'_m3" value="'.(isset($value['m3']) ? $value['m3'] : '').'" '.$disabled.' '.$attr.' /></label><label>&nbsp;'.L('立方米').'&nbsp;&nbsp;&nbsp;&nbsp;</label>';
		$str.= '<label>'.L('重量').'：</label><label><input size=5 class="form-control" type="text" name="data['.$name.'][kg]" id="dr_'.$name.'_kg" value="'.(isset($value['kg']) ? $value['kg'] : '').'" '.$disabled.' '.$attr.' /></label><label>&nbsp;'.L('千克').'</label>';
		$str.= $tips;
