...
class F_Linkage extends A_Field {
    ...
    public function __construct() {
		parent::__construct();
		$this->name = IS_ADMIN ? L('联动菜单') : ''; // 字段名称
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		if ($value && !$disabled) {
			...
			if (IS_ADMIN) {
				$str.= '<span class="form-control-static" id="dr_linkage_'.$name.'_cxselect">'.dr_linkagepos($cfg['option']['linkage'], $value, ' » ').'&nbsp;&nbsp;<a href="javascript:;" onclick="dr_linkage_select_'.$name.'()" style="color:blue">'.L('[重新选择]').'</a></span>';
			} else {
				$str.= '<div class="form-control-static" id="dr_linkage_'.$name.'_cxselect">'.dr_linkagepos($cfg['option']['linkage'], $value, ' » ').'&nbsp;&nbsp;<a href="javascript:;" onclick="dr_linkage_select_'.$name.'()" style="color:blue">'.L('[重新选择]').'</a></div>';
			}
		}
		$str.= '
		<script type="text/javascript">
			...
			$(function(){
				...
				$ld5.ld({ajaxOptions:{"url": "/member.php?&c=api&m=linkage&code='.$cfg['option']['linkage'].'"},defaultParentId:0})
