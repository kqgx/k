...
class F_Checkbox extends A_Field {
	...
	public function option($option) {
		...
		return '
			<div class="form-group">
				<label class="col-md-2 control-label">'.L('选项列表').'：</label>
				<div class="col-md-9">
					<textarea class="form-control" name="data[setting][option][options]" style="height:150px;width:400px;">'.$option['options'].'</textarea>
					<span class="help-block">'.L('格式：选项名称|选项值[回车换行]选项名称2|值2....').'</span>
				</div>
			</div>
			<div class="form-group">
				<label class="col-md-2 control-label">'.L('默认选中选项').'：</label>
				<div class="col-md-9">
					...
					<span class="help-block">'.L('默认选中项，多个选中项用|分隔').'</span>
				</div>
			</div>
			<div class="form-group">
	...
	public function insert_value($field) {
		$this->ci->data[$field['ismain']][$field['fieldname']] = array2string($this->ci->post[$field['fieldname']]);
	}
