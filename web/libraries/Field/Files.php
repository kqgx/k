...
class F_Files extends A_Field {
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		//(IS_ADMIN && IS_PC) || (isset($cfg['option']['is_swfupload']) && $cfg['option']['is_swfupload'])
			...
			//pc端采用Flash上传
			$url = '/member.php?c=api&m=upload'.$param;
			$file_value = '';
			$value && $value = string2array($value); // 字段默认值
			// 默认值输出
			if ($value && isset($value['file'])) {
				foreach ($value['file'] as $id => $fileid) {
					$title = $value['title'][$id];
					if (is_numeric($fileid)) {
						$edit = '<label><a href="javascript:;" class="btn btn-sm green" title="'.L('修改').'" onclick="dr_edit_file(\''.$url.'&count=1\',\''.$name.'\',\'999'.$id.'\')"><i class="fa fa-edit"></i></a></label>';
					} else {
						$edit = '<label><a href="javascript:;" class="btn btn-sm green" title="'.L('修改').'" onclick="dr_edit_input_file(\''.$fileid.'\',\''.$title.'\',\''.$name.'\',\'999'.$id.'\')"><i class="fa fa-edit"></i></a></label>';
					}
					$file_value.= '
					<li id="files_'.$name.'_999'.$id.'" list="999'.$id.'" style="cursor:move;">
						...
						<label><a href="javascript:;" class="btn btn-sm red" title="'.L('删除').'" onclick="dr_remove_file(\''.$name.'\',\'999'.$id.'\')"><i class="fa fa-trash"></i></a></label>
						'.$edit.'
			...
			if (!$disabled) {
				$str.= '<div id="drag-and-drop-zone-'.$name.'" class="btn btn-sm blue uploader" style="cursor: pointer;"><i class="fa fa-cloud-upload"></i> '.L('上传').' <input type="file" name="file" multiple=""></div>';
                $str.= '<a style="margin-left: 10px;margin-right:15px" class="btn blue btn-sm" href="javascript:;" onClick="dr_upload_files(\'' . $name . '\',\'' . $url . '\', \'\', \'' . (int)$cfg['option']['count'] . '\')"> <i class="fa fa-folder"></i> ' . L('浏览') . '</a>&nbsp;&nbsp;';
				...
				<script type="text/javascript">
				  ...
				  $("#drag-and-drop-zone-'.$name.'").dmUploader({
					url: "/member.php?c=api&m=new_ajax_upload'.$param.'",
					dataType: "json",
					allowedTypes: "*",
					onInit: function(){
