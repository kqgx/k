	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		$disabled = !IS_ADMIN && $id && $value && isset($cfg['validate']['isedit']) && $cfg['validate']['isedit'] ? 'disabled' : '';
			...
			if ($value) {
				...
				if ($data) {
					$my = '
					<button type="button" style="cursor:pointer;" class="btn green btn-sm" onclick="dr_show_file_info(\''.$data['id'].'\')"> <i class="fa fa-search"></i> '.L('预览').'</button>
			...
			if (!$disabled) {
                ...
                $str.= '<div style="margin-left: 10px;margin-right:15px" class="col-md-3 my_upload"><button type="button" style="cursor:pointer;"  class="btn blue btn-sm" onclick="dr_upload_file(\''.$name.'\', \''.$furl.'\')"> <i class="fa fa-folder"></i> ' . L('浏览') . '</button></div>';
			...
			<span class="help-block">'.($cfg['option']['resolution']? '分辨率：'.$cfg['option']['resolution']:'').'</span>
      ...
      $("#drag-and-drop-zone-'.$name.'").dmUploader({
        ...
        onUploadSuccess: function(id, data){
		  if (data.code == 1) {
			...
			$("#dr_my_'.$name.'_list").append("<button type=\"button\" style=\"cursor:pointer;\" class=\"btn red btn-sm\" onclick=\"dr_delete_file2(\''.$name.'\')\"> <i class=\"fa fa-trash\"></i> '.L('删除').'</button> ");
		  } else {
		  	dr_tips(data.msg,5);
