...
class F_Ueditor extends A_Field {
    ...
    public function option($option) {
        ...
        return '<div class="form-group">
                    <label class="col-md-2 control-label">'.L('宽度').'：</label>
                    <div class="col-md-9">
                        <label><input type="text" class="form-control" name="data[setting][option][width]" value="'.$option['width'].'"></label>
					    <span class="help-block">'.L('[整数]表示固定宽带；[整数%]表示百分比').'</span>
                    </div>
                </div>
				<div class="form-group">
                    <label class="col-md-2 control-label">'.L('高度').'：</label>
                    <div class="col-md-9">
				...
				<div class="form-group">
                    <label class="col-md-2 control-label">'.L('参数配置').'：</label>
                    <div class="col-md-9">
                    <fieldset class="blue pad-10">
						<legend>'.L('参数配置').'</legend>
						<div class="form-group">
							<div class="form-group">
								<label class="col-md-3 control-label">'.L('固定工具栏').'：</label>
								<div class="col-md-9">
                                <div class="radio-list">
								    <label class="radio-inline"><input type="radio" value="1" name="data[setting][option][autofloat]" '.($option['autofloat'] == 1 ? 'checked' : '').' > '.L('开启').'</label>
								    <label class="radio-inline"><input type="radio" value="0" name="data[setting][option][autofloat]" '.($option['autofloat'] == 0 ? 'checked' : '').' > '.L('关闭').'</label>
								</div>
								</div>
							</div>
							<div class="form-group">
								<label class="col-md-3 control-label">'.L('自动伸长高度').'：</label>
								<div class="col-md-9">
                                <div class="radio-list">
								    <label class="radio-inline"><input type="radio" value="1" name="data[setting][option][autoheight]" '.($option['autoheight'] == 1 ? 'checked' : '').' > '.L('开启').'</label>
								    <label class="radio-inline"><input type="radio" value="0" name="data[setting][option][autoheight]" '.($option['autoheight'] == 0 ? 'checked' : '').' > '.L('关闭').'</label>
								</div>
								</div>
							</div>
							<div class="form-group">
								<label class="col-md-3 control-label">'.L('远程图片模式').'：</label>
								<div class="col-md-9">
                                <div class="radio-list">
								    <label class="radio-inline"><input type="radio" value="1" name="data[setting][option][autodown]" '.($option['autodown'] == 1 ? 'checked' : '').' > '.L('队列（高效）').'</label>
								    <label class="radio-inline"><input type="radio" value="2" name="data[setting][option][autodown]" '.($option['autodown'] == 2 ? 'checked' : '').' > '.L('自动（会影响发布速度）').'</label>
								    <label class="radio-inline"><input type="radio" value="0" name="data[setting][option][autodown]" '.($option['autodown'] == 0 ? 'checked' : '').' > '.L('关闭').'</label>
								</div>
								</div>
							</div>
							<div class="form-group">
								<label class="col-md-3 control-label">'.L('Div标签转换为P标签').'：</label>
								<div class="col-md-9">
                                <div class="radio-list">
								    <label class="radio-inline"><input type="radio" value="0" name="data[setting][option][divtop]" '.($option['divtop'] == 0 ? 'checked' : '').' > '.L('开启').'</label>
								    <label class="radio-inline"><input type="radio" value="1" name="data[setting][option][divtop]" '.($option['divtop'] == 1 ? 'checked' : '').' > '.L('关闭').'</label>
								</div>
								</div>
							</div>
							<div class="form-group">
								<label class="col-md-3 control-label">'.L('分页标签').'：</label>
								<div class="col-md-9">
                                <div class="radio-list">
								    <label class="radio-inline"><input type="radio" value="1" name="data[setting][option][page]" '.($option['page'] ? 'checked' : '').' > '.L('开启').'</label>
								    <label class="radio-inline"><input type="radio" value="0" name="data[setting][option][page]" '.(!$option['page'] ? 'checked' : '').' > '.L('关闭').'</label>
								</div>
								</div>
							</div>
						</div>
				...
				<div class="form-group">
                    <label class="col-md-2 control-label">'.L('后台编辑器模式').'：</label>
                    <div class="col-md-9">
                        <div class="radio-list">
                            <label class="radio-inline"><input type="radio" value="1" name="data[setting][option][mode]" '.($option['mode'] == 1 ? 'checked' : '').' onclick="$(\'#bjqms1\').hide()"> '.L('完整').'</label>
                            <label class="radio-inline"><input type="radio" value="2" name="data[setting][option][mode]" '.($option['mode'] == 2 ? 'checked' : '').' onclick="$(\'#bjqms1\').hide()"> '.L('精简').'</label>
                            <label class="radio-inline"><input type="radio" value="3" name="data[setting][option][mode]" '.($option['mode'] == 3 ? 'checked' : '').' onclick="$(\'#bjqms1\').show()"> '.L('自定义').'</label>
                        </div>
                    </div>
                </div>
				<div class="form-group" id="bjqms1" '.($option['mode'] < 3 ? 'style="display:none"' : '').'>
                    <label class="col-md-2 control-label">'.L('工具栏').'：</label>
                    <div class="col-md-9">
                    <textarea name="data[setting][option][tool]" style="height:90px;" class="form-control">'.$option['tool'].'</textarea>
					<span class="help-block">'.L('必须严格按照Ueditor工具栏格式：\'fullscreen\', \'source\', \'|\', \'undo\', \'redo\'').'</span>
                    </div>
                </div>
				<div class="form-group">
                    <label class="col-md-2 control-label">'.L('前端编辑器模式').'：</label>
                    <div class="col-md-9">
                        <div class="radio-list">
                            <label class="radio-inline"><input type="radio" value="1" name="data[setting][option][mode2]" '.($option['mode2'] == 1 ? 'checked' : '').' onclick="$(\'#bjqms2\').hide()"> '.L('完整').'</label>
                            <label class="radio-inline"><input type="radio" value="2" name="data[setting][option][mode2]" '.($option['mode2'] == 2 ? 'checked' : '').' onclick="$(\'#bjqms2\').hide()"> '.L('精简').'</label>
                            <label class="radio-inline"><input type="radio" value="3" name="data[setting][option][mode2]" '.($option['mode2'] == 3 ? 'checked' : '').' onclick="$(\'#bjqms2\').show()"> '.L('自定义').'</label>
                        </div>
                    </div>
                </div>
				...
				<div class="form-group">
                    <label class="col-md-2 control-label">'.L('移动端编辑器模式').'：</label>
                    <div class="col-md-9">
                        <div class="radio-list">
                            <label class="radio-inline"><input type="radio" value="1" name="data[setting][option][mode3]" '.($option['mode3'] == 1 ? 'checked' : '').' onclick="$(\'#bjqms3\').hide()"> '.L('完整').'</label>
                            <label class="radio-inline"><input type="radio" value="2" name="data[setting][option][mode3]" '.($option['mode3'] == 2 ? 'checked' : '').' onclick="$(\'#bjqms3\').hide()"> '.L('精简').'</label>
                            <label class="radio-inline"><input type="radio" value="3" name="data[setting][option][mode3]" '.($option['mode3'] == 3 ? 'checked' : '').' onclick="$(\'#bjqms3\').show()"> '.L('自定义').'</label>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    <label class="col-md-2 control-label">'.L('默认值').'：</label>
                    <div class="col-md-9">
                        ...
                        <span class="help-block">'.L('也可以设置会员表字段，表示用当前登录会员信息来填充这个值').'</span>
                    </div>
                </div>
    ...
    public function insert_value($field) {
        ...
        if (isset($field['setting']['option']['autodown'])
            ...
            if ($uid == $this->ci->uid
                ...
                if ($this->ci->member['adminid']
                    ...
                    // 附件总空间不足
                    $this->ci->models('member/notice')->add($uid, 1, L('附件可用空间不足，无法下载远程图片'));
                }
            }
            // 处理远程图片
            ...
            foreach ($imgs[3] as $i => $img) {
                ...
                if ($down && $uid) {
					if ($field['setting']['option']['autodown'] == 1) {
						// 队列下载
						$id = $this->ci->models('system/attachment')->cron_catcher($uid, $img, $field);
						if ($id) {
							$timg = 0;
						...
						if (!$file) {
							...
							if (@file_put_contents($path.$filename.'.'.$fileext, $file)) {
								...
								$result = $this->ci->models('system/attachment')->upload($uid, $info);
								if (is_array($result)) {
									$id = $result[0];
									$file = $result[1];
        ...
        if ($field['fieldname'] == 'content'
            ...
            if ($attach) {
                ...
                // 筛选是否存在图片
                if ($img[3] && strpos($img[3], '/ueditor/') == false) {
    ...
    public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		<script type=\"text/javascript\">
			var editorOption = {
				...
				UEDITOR_URL: \"/statics/ueditor/\",
				serverUrl: \"/ueditor/index?siteid=".SITE_ID."&\",
				toolbars: [
					[ $tool ]
				],
				lang: \"".SITE_LANGUAGE."\",
