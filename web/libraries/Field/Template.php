...
class F_Template extends A_Field {
    ...
    public function __construct() {
		parent::__construct();
		$this->name = IS_ADMIN ? L('模板文件') : ''; // 字段名称
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
        ...
        if (APP_DIR && APP_DIR != 'member' && is_dir(WEBPATH.'module/'.APP_DIR)) {
            // 模块
            $files = directory_map(VIEWPATH.'pc/web/'.SITE_TEMPLATE.'/'.APP_DIR.'/', 1);
        } elseif (APP_DIR == 'member' || IS_MEMBER) {
            // 会员中心
            $files = directory_map(VIEWPATH.'pc/member/'.SITE_TEMPLATE.'/common/', 1);
        } else {
            // 网站
            $files = directory_map(VIEWPATH.'pc/web/'.SITE_TEMPLATE.'/common/', 1);
        }
        if ($files) {
			foreach ($files as $t) {
				if ($t && strpos($t, '.html')) {
