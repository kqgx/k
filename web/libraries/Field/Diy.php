...
class F_Diy extends A_Field {
	...
	public function option($option) {
        ...
        $files = directory_map(WEBPATH.'data/file/', 1);
        if ($files) {
            foreach ($files as $t) {
		...
		return '<div class="form-group">
                    ...
                    <label class="radio-inline"><input type="radio" onclick="dr_diy_type(0)" value="0" name="data[setting][option][type]" '.(!$option['type'] ? 'checked' : '').'> '.L('代码模式').'</label>
                    <label class="radio-inline"><input type="radio" onclick="dr_diy_type(1)" value="1" name="data[setting][option][type]" '.($option['type'] ? 'checked' : '').'> '.L('文件模式').'</label>
                    </div>
                    </div>
                </div>
                <div class="form-group" id="dr_diy_type_0" style="display:none">
                    <label class="col-md-2 control-label">'.L('代码').'：</label>
                    <div class="col-md-9">
                    <textarea class="form-control" name="data[setting][option][code]" style="height:150px;width:80%;">'.$option['code'].'</textarea>
					<span class="help-block">'.L('将设计好的代码放到这里').'</span>
                    </div>
                </div>
                <div class="form-group" id="dr_diy_type_1" style="display:none">
                    <label class="col-md-2 control-label">'.L('文件').'：</label>
                    <div class="col-md-9">
                    <label>'.$str.'</label>
                    <span class="help-block">'.L('将设计好的文件上传到/cache/field/目录之下').'</span>
                    </div>
                </div>
