...
class F_Fees extends A_Field {
	...
	public function option($option) {
		...
		return '<div class="form-group">
                ...
                <div class="form-group">
                    <label class="col-md-2 control-label">'.L('收费规则').'：</label>
                    <div class="col-md-9">
                        <div class="radio-list">
                            <label class="radio-inline"><input type="radio" value="0" name="data[setting][option][mode]" '.(!$option['mode'] ? 'checked' : '').'> '.L('按会员组级别设置').'</label>
                            <label class="radio-inline"><input type="radio" value="1" name="data[setting][option][mode]" '.($option['mode'] ? 'checked' : '').'> '.L('按会员组设置').'</label>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-md-2 control-label">'.L('适用范围').'：</label>
                    <div class="col-md-9">
                        <div class="form-control-static">'.L('此字段只能用于模块内容和模块扩展内容').'</div>
                    </div>
                </div>';
	}
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		// 字段默认值
        $value = $value ? string2array($value) : NULL;
