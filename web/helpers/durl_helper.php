...
function dr_url_prefix($url, $domain = '', $siteid = SITE_ID) {
    ...
    $is_pc = $ci->is_mobile_html ? 0 : IS_PC;

    // 判断是否是模块
    if ($domain && is_dir(WEBPATH.'module/'.$domain.'/') && strpos($url, 'http') === false) {
...
function dr_mobile_show_url($dir, $id, $page = NULL) {
    return '/index.php?'.(!$dir ? '' : 's='.$dir.'&').'c=show&id='.$id.($page ? '&page='.$page : '');
}
...
function dr_mobile_extend_url($dir, $id, $page = NULL) {
    return '/index.php?'.(!$dir ? '' : 's='.$dir.'&').'c=extend&id='.$id.($page ? '&page='.$page : '');
}
...
function dr_mobile_category_url($dir, $id, $page = NULL) {
    return '/index.php?'.(!$dir ? '' : 's='.$dir.'&').'c=category&id='.$id.($page ? '&page='.$page : '');
}
...
function dr_show_seo($mod, $data, $page = 1) {
	...
	$meta_title = $cat['setting']['seo']['show_title'] ? $cat['setting']['seo']['show_title'] : '['.L('第%s页', '{page}').'{join}]{title}{join}{name}{join}{modulename}{join}{SITE_NAME}';
...
function dr_category_seo($mod, $cat, $page = 1) {
	...
	$meta_title = $cat['setting']['seo']['list_title'] ? $cat['setting']['seo']['list_title'] : '['.L('第%s页', '{page}').'{join}]{modulename}{join}{SITE_NAME}';
...
function dr_search_seo($mod, $catid, $param, $page = 1) {
	...
	$meta_title = $mod['site'][SITE_ID]['search_title'] ? $mod['site'][SITE_ID]['search_title'] : '['.L('第%s页', '{page}').'{join}][{keyword}{join}][{param}{join}]{modulename}{join}{SITE_NAME}';
    ...
    $seo['meta_title'] = safe_replace($seo['meta_title']);
    $seo['meta_keywords'].= $mod['site'][SITE_ID]['meta_keywords'];
    $seo['meta_keywords'] = safe_replace(trim($seo['meta_keywords'], ','));
 ...
 * @return	string	地址
 */
function dr_member_url($url = '', $query = array(), $self = 'member.php') {
    ...
    if (defined('MEMBER_URL_RULE') || strpos(MEMBER_URL, 'member.php') !== false) {
        // 未绑定域名的情况下
