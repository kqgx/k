...
function dr_html_auth($is = 0)
{
    ...
    $file = DATAPATH . 'views/' . md5($_SERVER['HTTP_USER_AGENT']) . '.auth';
    if ($is) {
        // 存储值
...
function dr_get_theme()
{
    ...
    return array_diff(dr_dir_map(STATICS, 1),
        array('avatar', 'admin', 'comment', 'emotions', 'js', 'Oauth', 'watermark'));
...
function dr_member_info($uid, $cache = -1)
{
    ...
    if (!$data) {
        $data = $ci->models('member')->get_member($uid);
...
{
    return "<script type=\"text/javascript\" src=\"" . SITE_URL . "member.php?c=api&m=online&uid={$uid}&type={$type}\"></script>";
}
 ...
 * @return    string
 */
function dr_avatar($uid, $size = '180')
{
...
function dr_thumb($img, $width = null, $height = null, $water = 0, $size = 0)
{
    ...
    if (is_numeric($img)) { // 表示附件id
        ...
        $thumb_file = trim(SYS_THUMB_DIR,
                '/') . '/' . md5($img) . '/' . dr_safe_filename("$width-$height-$water-$size") . '.jpg';
        if (is_file(MTBASE . $thumb_file)) {
...
function dr_down_file($id)
{
    ...
    if (is_numeric($id)) { // 表示附件id
        ...
        if ($info) {
            return SITE_URL . "member.php?c=api&m=file&id=$id";
        }
    }
...
function dr_file($url)
{
    ...
    return SITE_URL . SYS_ATTACHMENT_URL . $url;
}
...
{
    return get_instance()->get_cache('var', $name);
}
...
{
    return $keyword != '' ? str_ireplace($keyword, '<font color=red><strong>' . $keyword . '</strong></font>',
        $string) : $string;
}
...
function dr_array2xml($arr, $htmlon = true, $isnormal = false, $level = 1)
{
    ...
    foreach ($arr as $k => $v) {
        if (!is_array($v)) {
            ...
            $string .= $space . "<$name>\r\n" . dr_array2xml($v, $htmlon, $isnormal,
                    $level + 1) . $space . "</$name>\r\n";
        }
    }
...
// 二维码
function dr_qrcode_url($text, $uid = 0, $level = QR_ECLEVEL_L, $size = 5, $margin = 4)
{
    return SITE_URL . 'index.php?c=api&m=qrcode&uid=' . urlencode($uid) . '&text=' . urlencode($text) . '&size=' . $size . '&level=' . $level . '&margin=' . $margin;
}
...
var status = 0, result, apiFail;
            ...
            $.ajax({url : "/member.php?c=api&m=geetest&rand="+Math.round(Math.random()*100),
                type : "get",
                dataType : "JSON",
...
function dr_vip_form($id, $form, $field = 0, $sid = 0, $cache = 0)
{
    ...
    if (!$data) {
        ...
        $data2 = $ci->db->where('id',
            $id)->get($sid . '_form_' . $form . '_data_' . (int)$data['tableid'])->row_array();
        if ($data2) {
...
function dr_vip_mform($id, $dir, $form, $field = 0, $sid = 0, $cache = 0)
{
    ...
    if (!$data) {
        ...
        $data2 = $ci->db->where('id',
            $id)->get($sid . '_' . $dir . '_form_' . $form . '_data_' . (int)$data['tableid'])->row_array();
        if ($data2) {
            $data = array_merge($data, $data2);
        }
        if (!$data) {
...
// 替换菜单名称
function dr_replace_m_name($f, $t)
{
    if (strpos($f, '{name}') !== false) {
        return str_replace('{name}', $t, $f);
    }
    return $f;
}
...
function dr_format_html_file($file, $is_mobile = 0)
{
    ...
    // 多网站时生成到指定目录
    if (SITE_ID > 1 && is_file(CONFPATH . 'html.php')) {
        $html = require CONFPATH . 'html.php';
...
function dr_tpl_path($file)
{
    ...
    $path = IS_MEMBER ? VIEWPATH . (IS_MOBILE ? 'mobile' : 'pc') . '/member/' . MEMBER_TEMPLATE . '/' : VIEWPATH . (IS_MOBILE ? 'mobile' : 'pc') . '/web/' . SITE_TEMPLATE . '/';
...
function dr_field_form($id, $value = '', $html = '{value}')
{
    ...
    return preg_replace('/(<div class="on.+<\/div>)/U', '',
        $field->input($data['name'], $data['fieldname'], $data['setting'], $value, 0));
}
...
function dr_mall_property($catid, $all = 0, $mdir = APP_DIR)
{
    ...
    if (!$mdir || !is_dir(WEBPATH . 'module/' . $mdir . '/')) {
        return null;
    }

    $ci = &get_instance();
    $ci->load->add_package_path(WEBPATH . 'module/' . $mdir . '/');
        ...
        if ($t['list']) {
            ...
            foreach ($t['list'] as $a) {
                ...
                if ($a['type'] == 'text') {
                    ...
                    $a['search'] && $row = '<a href="' . dr_search_url($params, 'mall_' . $a['cname'],
                            $value[$a['cname']]) . '" target="_blank" class="dr_pt">' . $row . '</a>';
                } else {
                    $row = array();
                    foreach ($value[$a['cname']] as $v) {
                        $row[$v] = $a['search'] ? '<a href="' . dr_search_url($params, 'mall_' . $a['cname'],
                                $v) . '" target="_blank" class="dr_pt">' . $a['value'][$v] . '</a>' : $a['value'][$v];
                    }
                }
                $list[$i][$a['name']] = $row;
            }
...
function dr_mall_spec($catid, $value = array(), $mdir = 'mall')
{
    ...
    if (!$catid) {
        return null;
    } elseif (!is_dir(WEBPATH . 'module/' . $mdir . '/')) {
        return null;
    }
...
function dr_order_users($dir, $id)
{
    ...
    if (!is_dir(FCPATH . 'module/order/') || !is_dir(WEBPATH . 'module/' . $dir)) {
        return 0;
    }
...
function dr_get_spec($name, $catid, $value)
{
    ...
    foreach ($data as $t) {
        ...
        foreach ($t['value'] as $a => $b) {
            ...
            $sl = isset($value['option'][$t['cname']]) && dr_in_array($a,
                $value['option'][$t['cname']]) ? ' checked' : '';
        ...
        $html .= '<div class="sku-li">
					...
					<div style="clear:both;"></div>
					<div class="Father_Footer"><input style="margin-top:-5px !important" onclick="dr_spec_add_value(this, \'' . $t['cname'] . '\')" class="toggle md-check" type="checkbox" value="" />&nbsp;<label><input id="dr_spec_add_value_' . $t['cname'] . '" placeholder="' . L('自定义') . '" class="input-text" size="8" type="text" value=""></label>&nbsp;&nbsp;&nbsp;</div>
				</div>';
        $i++;
    }
...
function swiper($data, $h = 850, $w = 1150)
{
    if (empty($data)) {
        return false;
    }
    return dr_thumb(json_decode($data, true)[0], $h, $w);
}

function fileName($id)
{
    $ci = &get_instance();
    return $info = $ci->get_cache_data("attachment-{$id}");
}
