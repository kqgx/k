...
function dr_reply_show(id, username) {
	var html = $("#dr_post_form").html();
	html = html.replace("myform", "myform_" + id);
	html = html.replace("dr_review_post", "dr_review_post_" + id);
	html = html.replace("dr_post_comment()", "dr_reply_comment(" + id + ")");
	$(".ds-replybox2").hide();
	$("#dr_reply_" + id).html(html);
	var obj = $("#myform_" + id + " textarea[name='content']");
	obj.val("@" + username + "  ");
	obj.focus();
	$("#dr_reply_" + id).show();
	dr_show_bq();
...
function dr_zc_comment(id) {
	$.ajax({
		type: "POST",
		dataType: "json",
		url: comment_url + "&m=options&option=support&id=" + id,
		data: {},
		success: function(res) {
			if (res.code) {
				$("#dr_comment_zc_" + id).html(res.data)
			} else {
				dr_tips(res.msg)
			}
		},
...
function dr_fd_comment(id) {
	$.ajax({
		type: "POST",
		dataType: "json",
		url: comment_url + "&m=options&option=oppose&id=" + id,
		data: {},
		success: function(res) {
			if (res.code) {
				$("#dr_comment_fd_" + id).html(res.data)
			} else {
				dr_tips(res.msg)
			}
		},
