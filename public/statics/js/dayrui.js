...
function dr_input_files(name, count) {
    ...
    var id = size + 1;
    var url = '/member.php?c=api&&m=upload_input';
    $.ajax({
        type: "GET",
        url: url,
        dataType: "text",
...
function dr_edit_input_file(file2, title2, name, id) {
    var url = '/member.php?c=api&m=upload_input&file=' + file2 + '&title=' + title2;
    $.ajax({
        type: "GET",
        url: url,
        dataType: "text",
        ...
        ok: function() {
            ...
            if (value == "" || value == undefined) {
                ...
                if (iframe.$("#is_down").attr("checked")) {
                    var url2 = memberpath + "member.php?c=api&m=down_file";
...
function dr_login() {
    top.dialog({
        quickClose: true,
        url: memberpath + "member.php?c=login&m=ajax",
        title: lang["login"],
        opacity: 0.1,
        lock: true,
        width: 380,
...
function dr_chat(_this) {
    ...
    var url = memberpath + "member.php?c=api&m=webchat&username=" + username + "&uid=" + uid + "&online=" + online + "&" + Math.random();
    $.ajax({
        type: "GET",
        url: url,
        dataType: "jsonp",
...
function dr_show_file_info(name) {
    var url = memberpath + "member.php?c=api&m=fileinfo&name=" + name + "&rand=" + Math.random();
    var d = top.dialog({
        title: lang["fileinfo"],
        fixed: true,
        url: url,
