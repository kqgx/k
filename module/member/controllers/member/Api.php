...
class Api extends M_Controller {
    ...
    public function ajax_delete_draft() {
        ...
        if ($this->site[$sid]->where('id', $did)->where('uid', $this->uid)->get($table)->row_array()) {
            ...
            // 删除表对应的附件
            $this->models('system/attachment')->delete_for_table($table.'-'.$did);
            exit(''.$did.'');
        } else {
            exit('删除失败：草稿不存在！');
        }
    ...
    public function related() {
        ...
        // 登陆判断
        !$this->uid && $this->admin_msg(L('会话超时，请重新登录'));

        // 参数判断
        $dirname = safe_replace($this->input->get('module'));
        !$dirname && $this->admin_msg(L('模块module参数不存在'));
        ...
        !$module && $this->admin_msg(L('此模块（%s）缓存不存在', $dirname));
        ...
        if (IS_POST) {
            ...
            if (isset($data['keyword']) && $data['keyword']
                ...
                $data['keyword'] = safe_replace(urldecode($data['keyword']));
                if ($data['field'] == 'id') {
                    // id搜索
                    $id = array();
    ...
    public function notice() {
        ...
        $callback = isset($_GET['callback']) ? safe_replace($this->input->get('callback', TRUE)) : 'callback';
    ...
    public function userinfo() {
        ...
        $callback = safe_replace($this->input->get('callback', TRUE));
        if ($callback) {
    ...
    public function field() {
        ...
        $fields = $this->models('system/field')->get_data();
        $related = $this->input->post('relatedname');
        if ($data) {
            $value = string2array($data['setting']);
            $value = $value['option'];
        } else {
            $value = array();
        }
    ...
    public function upload() {
        ...
        $uid = $this->uid;
        // 附件上传时采用后台登陆会员
        $this->session->userdata('member_auth_uid') && $uid = $this->models('member')->member_uid(1);


        $my = $this->models('system/attachment')->get_my_attach($uid, $ext);
        $notused = $this->models('system/attachment')->get_unused($uid, $ext);
    ...
    public function fex_ajax_upload() {
        ...
        !$this->member && exit('0,'.L('游客不允许上传附件'));

        // 会员组权限
        $member_rule = $this->get_cache('member', 'setting', 'permission', $this->member['mark']);

        // 是否允许上传附件
        !$this->member['adminid'] && !$member_rule['is_upload'] && exit('0,'.L('抱歉！您的会员组无权上传附件'));
        ...
        if (!$this->member['adminid'] && $member_rule['attachsize']) {
            ...
            $filesize > $member_rule['attachsize'] * 1024 * 1024 && exit('0,'.L('附件空间不足！您的附件总空间%s，现有附件%s。', $member_rule['attachsize'].'MB', dr_format_file_size($filesize)));
        }
        ...
        if (IS_POST) {
            ...
            if ($this->upload->do_upload('file')) {
                ...
                $this->models('system/attachment')->siteid = $site ? $site : SITE_ID;
                $result = $this->models('system/attachment')->upload($uid, $info);
    ...
    public function sns_upload() {
        ...
        !$uid && exit(json_encode(array('status' => 0, 'data' => L('游客不允许上传附件'))));

        // 根据页面传入的session来获取当前登录uid，未获取到uid时提示游客无法上传
        $this->member = $this->models('member')->get_member($uid); // 获取会员信息

        // 游客不允许上传，未获取到会员信息时提示游客无法上传
        !$this->member && exit(json_encode(array('status' => 0, 'data' => L('游客不允许上传附件'))));

        // 会员组权限
        $member_rule = $this->get_cache('member', 'setting', 'permission', $this->member['mark']);

        // 是否允许上传附件
        !$this->member['adminid'] && !$member_rule['is_upload'] && exit(json_encode(array('status' => 0, 'data' => L('抱歉！您的会员组无权上传附件'))));

        // 附件总大小判断
        if (!$this->member['adminid'] && $member_rule['attachsize']) {
            $data = $this->db->select_sum('filesize')->where('uid', $uid)->get('attachment')->row_array();
            $filesize = (int)$data['filesize'];
            $filesize > $member_rule['attachsize'] * 1024 * 1024 && exit(json_encode(array('status' => 0, 'data' => L('附件空间不足！您的附件总空间%s，现有附件%s。', $member_rule['attachsize'].'MB', dr_format_file_size($filesize)))));
        }
    ...
    public function ajax_upload() {
        ...
        // 游客不允许上传
        !$this->member && exit(json_encode(array('code'=>0, 'msg'=>L('游客不允许上传附件'), 'url'=> '', 'id'=>'')));

        // 会员组权限
        $member_rule = $this->get_cache('member', 'setting', 'permission', $this->member['mark']);

        // 是否允许上传附件
        !$this->member['adminid'] && !$member_rule['is_upload'] && exit(json_encode(array('code'=>0, 'msg'=>L('您的会员组无权上传附件'), 'url'=> '', 'id'=>'')));

        // 附件总大小判断
        if (!$this->member['adminid'] && $member_rule['attachsize']) {
            $data = $this->db->select_sum('filesize')->where('uid', $this->uid)->get('attachment')->row_array();
            $filesize = (int)$data['filesize'];
            $filesize > $member_rule['attachsize'] * 1024 * 1024 && exit(json_encode(array('code'=>0, 'msg'=>L('附件空间不足！您的附件总空间%s，现有附件%s。', $member_rule['attachsize'].'MB', dr_format_file_size($filesize)), 'url'=> '', 'id'=>'')));
        }
        ...
        if ($this->upload->do_upload(isset($_GET['fname']) ? $_GET['fname'] : 'Filedata')) {
            ...
            $this->models('system/attachment')->siteid = $site;
            $result = $this->models('system/attachment')->upload($this->uid, $info);
            !is_array($result) && exit('0,'.$result);
            list($id, $file, $_ext) = $result;
    ...
    public function down_file() {
        ...
        !$this->uid && $this->json(0, L('游客不允许上传附件'));

        // 会员组权限
        $member_rule = $this->get_cache('member', 'setting', 'permission', $this->member['mark']);

        // 是否允许上传附件
        !$this->member['adminid'] && !$member_rule['is_upload'] && $this->json(0, L('您的会员组无权上传附件'));

        // 附件总大小判断
        if (!$this->member['adminid'] && $member_rule['attachsize']) {
            $data = $this->db->select_sum('filesize')->where('uid', $this->uid)->get('attachment')->row_array();
            $filesize = (int)$data['filesize'];
            $filesize > $member_rule['attachsize'] * 1024 * 1024 && $this->json(0, L('附件空间不足！您的附件总空间%s，现有附件%s。', $member_rule['attachsize'].'MB', dr_format_file_size($filesize)));
        }
        ...
        !$file && $this->msg(0, '获取远程文件失败');


        $fileext = strtolower(trim(substr(strrchr($furl, '.'), 1, 10))); //扩展名
        $exts = (array)explode(',', $ext);
        !in_array($fileext, $exts) && $this->msg(0, '远程文件扩展名（'.$fileext.'）不允许');
        $fileext == 'php' && $this->msg(0, '远程文件扩展名（'.$fileext.'）不允许');
        ...
        if (@file_put_contents($path.$filename.'.'.$fileext, $file)) {
            ...
            $this->models('system/attachment')->siteid = $p['siteid'] ? $p['siteid'] : SITE_ID;
            $result = $this->models('system/attachment')->upload($this->uid, $info);
            if (is_array($result)) {
                list($id) = $result;
    ...
    public function swfupload() {
        ...
        // 是否允许上传附件
        !$this->member['adminid'] && !$member_rule['is_upload'] && exit('0,'.L('您的会员组无权上传附件'));
    ...
    public function new_ajax_upload() {
        ...
        // 游客不允许上传，未获取到会员信息时提示游客无法上传
        !$this->member && exit(json_encode(array('code'=>0, 'msg'=>L('游客不允许上传附件'))));

        // 会员组权限
        $member_rule = $this->get_cache('member', 'setting', 'permission', $this->member['mark']);

        // 是否允许上传附件
        !$this->member['adminid'] && !$member_rule['is_upload'] && exit(json_encode(array('code'=>0, 'msg'=>L('您的会员组无权上传附件'))));

        // 附件总大小判断
        if (!$this->member['adminid'] && $member_rule['attachsize']) {
            $data = $this->db->select_sum('filesize')->where('uid', $this->uid)->get('attachment')->row_array();
            $filesize = (int)$data['filesize'];
            $filesize > $member_rule['attachsize'] * 1024 * 1024 && exit(json_encode(array('code'=>0, 'msg'=>L('附件空间不足！您的附件总空间%s，现有附件%s。', $member_rule['attachsize'].'MB', dr_format_file_size($filesize)))));
        }
    ...
    public function swfdelete() {
        ...
        if ($data) {
            ...
            $this->models('system/attachment')->_delete_attachment($data);
        }
    }
    ...
    public function myattach() {
        ...
        !$this->member['adminid'] && exit(L('只有管理组才能附件浏览'));
    ...
    public function uemanager() {
        ...
        $data = $this->models('system/attachment')->get_unused($this->uid, 'jpg,png,gif|jpeg');
        if (!$data) {
            return NULL;
        }
    ...
    public function checktitle() {
        ...
        $row ? exit(L('<a href="'.$row['url'].'" style="color:red" target="_blank">'.L('已经存在').'</a>')) : exit('');
    }
    ...
    public function getkeywords() {
        ...
        $kw = safe_replace($kw ? $kw : $this->input->get('title'));
    ...
    public function fileinfo() {
        ...
        $key = safe_replace($this->input->get('name'));
        ...
        if (in_array($info['fileext'], array('jpg', 'jpeg', 'gif', 'png'))) {
            ...
            echo '<p style="text-align: center"><a target="_blank" href="'.$file.'">'.L('查看原图').'</a></p>';
        } else {
    ...
    public function file() {
        ...
        !$info && $this->admin_msg(L('附件不存在或者已经被删除'));
    ...
    public function oauth() {
        ...
        $this->uid && $this->member_msg(L('请退出后再登录'), $_SERVER['HTTP_REFERER']);

        $appid = $this->input->get('id');
        $oauth = require CONFPATH.'oauth.php';
        $config	= $oauth[$appid];
        !$config && $this->member_msg(L('OAuth错误: 缺少OAuth参数'));

        $code = $this->input->get('code', TRUE);
        $MEMBER = $this->get_cache('member');

		if (IS_POST) {
			$user = string2array(dr_authcode($this->input->post('code'), 'DECODE'));
            !$user && $this->member_msg(L('数据已过期，请重新登录'));
			//
			$data = $this->input->post('data');
			...
			if ($type == 1) {
				// 登录
				$rt = $this->models('member/login')->login($data['username'], $data['password'], 36000);
				if (strlen($rt) > 3) {
				    // 登录成功
                    !$this->uid && $this->member_msg(L('绑定失败，请重新登录'));
					// 绑定到此账号
					$user['uid'] = $this->uid;
					$this->db->insert('member_oauth', $user);
                    $this->hooks->call_hook('member_login', $data); // 登录成功挂钩点
					$this->member_msg(dr_weixin_emoji($user['nickname']).'，'.L('登录成功').$rt, dr_member_url('home/index'), 1, 1);
				} elseif ($rt == -1) {
					$error1 = L('会员不存在');
				} elseif ($rt == -2) {
					$error1 = L('密码不正确');
				} elseif ($rt == -3) {
					$error1 = L('Ucenter注册失败');
				} elseif ($rt == -4) {
					$error1 = L('Ucenter：会员名称不合法');
				}
			} else {
				// 注册
				$id = $this->models('member/register')->add($data);
				if ($id > 0) {
				    // 注册成功
                    $data['uid'] = $this->uid;
                    $this->hooks->call_hook('member_register_after', $data); // 注册之后挂钩点
                    // 注册后的登录
                    $rt = $this->models('member/login')->login($id, $data['password'], 86400, 0, 1);
					...
					$error = array('name' => 'username', 'msg' => L('该会员【%s】已经被注册', $data['username']));
				} elseif ($id == -2) {
					$error = array('name' => 'email', 'msg' => L('邮箱格式不正确'));
				} elseif ($id == -3) {
					$error = array('name' => 'email', 'msg' => L('该邮箱【%s】已经被注册', $data['email']));
				} elseif ($id == -4) {
					$error = array('name' => 'username', 'msg' => L('同一IP在限制时间内注册过多'));
				} elseif ($id == -5) {
					$error = array('name' => 'username', 'msg' => L('Ucenter：会员名称不合法'));
				} elseif ($id == -6) {
					$error = array('name' => 'username', 'msg' => L('Ucenter：包含不允许注册的词语'));
				} elseif ($id == -7) {
					$error = array('name' => 'username', 'msg' => L('Ucenter：Email格式有误'));
				} elseif ($id == -8) {
					$error = array('name' => 'username', 'msg' => L('Ucenter：Email不允许注册'));
				} elseif ($id == -9) {
					$error = array('name' => 'username', 'msg' => L('Ucenter：Email已经被注册'));
				} elseif ($id == -10) {
					$error = array('name' => 'phone', 'msg' => L('手机号码必须是11位的整数'));
				} elseif ($id == -11) {
					$error = array('name' => 'phone', 'msg' => L('该手机号码已经注册'));
				}
				$error2 = $error['msg'];
			}
			...
			$this->template->assign(array(
				'type' => $type,
				'code' => dr_authcode(array2string($user), 'ENCODE'),
				'Oauth' => $user,
				'error_1' => $error1,
				'error_2' => $error2,
        ...
        if (!$code) {
            ...
            try {
                ...
                $this->member_msg(L('OAuth授权错误').' _ '.$e);
            }
        } else {
            // 回调返回数据
            try {
                ...
                if (is_array($user) && $user['oid']) {
                    ...
                    $code = $this->models('member/oauth')->oauth($appid, $user);
					  if ($code == 'bang') {
							// 绑定账号
							$this->template->assign(array(
								'type' => 1,
							...
							// 直接注册
                    		$this->member_msg(dr_weixin_emoji($user['nickname']).'，'.L('登录成功').$code, dr_member_url('home/index'), 1, 3);
						}
                } else {
                    $this->member_msg(L('OAuth回调错误: 获取用户信息失败'));
                }
            } catch (OAuth2_Exception $e) {
                $this->member_msg(L('OAuth回调错误: 获取用户信息失败').' - '.$e);
            }
        }
    }
    ...
    public function login() {
        $data = $this->models('member/login')->login($this->input->get('username'), $this->input->get('password'), NULL, $this->input->get('back'));
        echo $data['email'];
    }
    ...
    public function webchat() {
        ...
        if ($this->input->get('action') == 'more') {
            ...
            (!$this->uid || !$this->member) && exit($callback . '(' . json_encode(array('status' => 0, 'msg' => L('会话超时，请重新登录'))) . ')');
    ...
    public function geetest() {
        ...
        require LIBRARIES.'Geetestlib.php';
