...
class Tag extends Extends_Table_Admin {
    ...
    public function __construct() {
        ...
        $this->template->assign(array(
            ...
            'menu' => $this->get_menu_v3(array(
                L('关键词库') => array('admin/tag/index', 'tag'),
                L('添加') => array('admin/tag/add', 'plus'),
                L('批量添加') => array('admin/tag/all_add', 'plus-square-o'),
                L('更新缓存') => array(APP_DIR.'/admin/tag/cache', 'refresh'),
                L('自定义字段') => array('admin/field/index/rname/tag/rid/'.SITE_ID, 'plus-square'),
            )),
        ));
    }
    ...
    public function all_add() {
        ...
        if (IS_POST) {
            $rt = $this->models('Module/tag')->save_all_data($this->pid, $_POST['all']);
    ...
    public function _insert_data($data) {
        ...
        $data['name'] = safe_replace($_POST['data']['name']);
        $data['code'] = safe_replace($_POST['data']['code']);
        $data['hits'] = intval($_POST['data']['hits']);

        if ($this->models('Module/tag')->check_code(0, $data['code'])) {
            $this->admin_msg(L('别名已经存在'));
        } elseif ($this->models('Module/tag')->check_name(0, $data['name'])) {
            $this->admin_msg(L('关键词名称已经存在'));
        }
    ...
    public function _update_data($id, $data, $_data) {
        ...
        if ($this->models('Module/tag')->check_code($id, $data['code'])) {
            $this->admin_msg(L('别名已经存在'));
        } elseif ($this->models('Module/tag')->check_name($id, $data['name'])) {
            $this->admin_msg(L('关键词名称已经存在'));
        }
    ...
    public function cache() {
        $this->models('Module/tag')->cache(isset($_GET['site']) && $_GET['site'] ? (int)$_GET['site'] : SITE_ID);
