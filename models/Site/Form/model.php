...
class Site_Form_model extends CI_Model {
    ...
    public function del($id, $sid = SITE_ID) {
        ...
        // 删除附件
        $this->models('system/attachment')->delete_for_table($table, TRUE);

        // 删除文件
        $name = 'Form_'.$data['table'];
        @unlink(FCPATH.'controllers/'.$name.'.php');
        @unlink(FCPATH.'controllers/admin/'.$name.'.php');
        @unlink(FCPATH.'views/admin/form_'.$id.'.html');
        ...
        foreach ($form as $t) {
            ...
            if ($data) {
                foreach ($data as $field) {
                    $field['setting'] = string2array($field['setting']);
            ...
            $uri = 'admin/form/data/form/'.$t['table'];
            $has = $this->db->where('uri', $uri)->get('admin_menu')->row_array();
            if ($has) {
                continue;
            }
            $leftid = 0;
            ...
            if (!$left) {
                ...
                if ($top) {
                    $this->models('site/menu')->set('admin')->add_admin_menu(array(
                        'uri' => '',
                        'pid' => $top['id'],
                        'mark' => 'content-form',
                        'name' => L('网站表单'),
    ...
    public function get_data($id, $table, $attached = TRUE) {
    ...
    /**
     * 分销申请通过
     * @param array $data 分销申请数据
     * @return void
     */
    public function distribution_application_approved_model($data)
    {
        $this->db->where('id', $data['id'])->update($this->prefix.'_distribution_application', ['status' => 1]);
        $this->db->where('uid', $data['uid'])->update('member', ['groupid' => 7, 'name' => $data['name'], 'sex' => $data['sex'], 'phone' => $data['phone'], 'common_address' => $data['common_address']]);
    }

    /**
     * 分销申请驳回
     * @param int $id 分销申请 id
     * @return void
     */
    public function distribution_application_rejected_model($id)
    {
        $this->db->where('id', $id)->update($this->prefix.'_distribution_application', ['status' => -1]);
    }

    /**
     * 提现申请状态
     * @param int $id 提现申请 id
     * @param int $status 状态码 0：未审核 1：通过 -1：驳回
     * @return void
     */
    public function withdraw_application_status($id, $status)
    {
        $this->db->where('id', $id)->update($this->prefix.'_withdrawal', ['status' => $status]);
    }
}