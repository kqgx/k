...
class Site_model extends CI_Model {
	...
	public function add($data) {
		...
		$data['setting']['SITE_NAVIGATOR'] = '默认链接';
    ...
    public function cache() {
        ...
        $oldfile = directory_map(CONFPATH.'site/');
        foreach ($oldfile as $file) {
            @unlink(CONFPATH.'site/'.$file);
        }
        ...
        foreach ($data as $id => $t) {
            ...
            // 写入配置文件
            $this->dconfig->file(CONFPATH.'site/'.$id.'.php')->note('站点配置文件')->space(32)->to_require_one($this->config, $t['setting'], 1);
            $this->dconfig->file(CONFPATH.'client.php')->note('站点终端配置文件')->space(32)->to_require_one($this->config, $t['setting'], 1);
            // 写入缓存文件
            $cache[$id] = $t['setting'];
        ...
        if ($data) {
            ...
            foreach ($data as $t) {
                // 排除不存在的模块
                if (!is_dir(WEBPATH.'module/'.$t['dirname'])) {
                    continue;
                }
                // 排除自定义数据的模块
                $cfg = require WEBPATH.'module/'.$t['dirname'].'/config/module.php';
        ...
        @unlink(CONFPATH.'module_domain.php');

        // 生成站点域名归属
        $this->dconfig->file(CONFPATH.'domain.php')->note('站点域名文件')->space(32)->to_require_one($domain);
        $this->dconfig->file(CONFPATH.'client_domain.php')->note('站点终端域名文件')->space(32)->to_require_one($client);
        $this->dconfig->file(CONFPATH.'module_domain.php')->note('模块域名归类文件')->space(32)->to_require_one($module_domain);
