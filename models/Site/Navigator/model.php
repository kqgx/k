...
class Site_Navigator_model extends CI_Model {
     ...
     * @return array
     */
    public function api_get_data($select = '*', $where = '')
    {
        if ($where) {
            $this->db->where($where);
        }
        return $this->db->select($select)->order_by('displayorder ASC,id ASC')->get($this->tablename)->result_array();
    }
    ...
    public function delete($ids) {
        ...
        if ($this->categorys) {
            ...
            foreach ($this->categorys as $id) {
                $this->models('system/attachment')->delete_for_table($this->tablename.'-'.$id);
            }
        }
    ...
    public function cache($site) {
        ...
        if ($data) {
            // 加载单页模型类 debug
            $page_model = $this->load->model('site/page');
            $page_model->tablename = $this->db->dbprefix($site.'_page');
            // 初始化数据
            $page = $cache = array();
            foreach ($data as $t) {
                ...
                if ($t['mark']) {
                    ...
                    if (strpos($t['mark'], 'page') === 0) {
                        ...
                        !$page && $page = $page_model->get_data_all();
                        if (isset($page[$id]) && $page[$id]) {
                            $url = $page_model->get_url($page[$id], $site);
    ...
    public function install(){
        "DROP TABLE IF EXISTS `imt_1_navigator`;
CREATE TABLE `imt_1_navigator` (
