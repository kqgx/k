...
class Navigator extends M_Controller {
    ...
    public function __construct() {
		...
		foreach ($data as $i => $name) {
			if ($name) {
                $use[$i] = $i;
				$menu[L($name)] = array('admin/navigator/index'.(isset($_GET['type']) || $i ? '/type/'.$i : ''), 'map-marker');
				$this->menu[$i] = L($name);
			}
		}
        // 设置默认选中
    ...
    public function index() {
		...
		if (IS_POST && $this->input->post('ids')) {
			...
			if ($this->input->post('action') == 'del') {
				...
				$this->models('site/navigator')->delete($ids);
                $this->cache(1);
		...
		$tree = array();
		$data = $this->models('site/navigator')->get_data($this->type);
		if ($data) {
			foreach($data as $t) {
				$add = dr_url('navigator/add', array('pid' => $t['id'], 'type' => $this->type));
				$edit = dr_url('navigator/edit', array('id' => $t['id'], 'type' => $this->type));
				$t['option'] = '';
				// $this->is_auth('admin/navigator/add') && $t['option'].= '<a class="aadd" title="'.L('添加').'" href="'.$add.'"> <i class="fa fa-plus"></i> '.L('添加').'</a>';
				$this->is_auth('admin/navigator/edit') && $t['option'].= '<a class="aedit" title="'.L('修改').'" href="'.$edit.'"> <i class="fa fa-edit"></i> '.L('修改').'</a>';
				$t['option'].= '<a class="ago" title="'.L('访问').'" href="'.$t['url'].'" target="_blank"> <i class="fa fa-paper-plane"></i> '.L('访问').'</a>';
                if (strpos($t['mark'], 'page') === 0) {
                    //1
                    $t['ntype'] = '<font color=blue>'.L('单页').'</font>';
                } elseif (strpos($t['mark'], 'module') === 0) {
                    //2
                    list($a, $dir, $catid) = explode('-', $t['mark']);
                    $t['ntype'] = '<font color=green>'.L('模块').'</font>';
                    if ($catid) {
                        $t['option'].= '<a class="aadd" href="'.dr_url($dir.'/category/add', array('id' => $catid)).'"> <i class="fa fa-plus"></i> '.L('添加栏目').'</a>';
                        $t['option'].= '<a class="aedit" href="'.dr_url($dir.'/category/edit', array('id' => $catid)).'"> <i class="fa fa-edit"></i> '.L('修改栏目').'</a>';
                    }
                } else {
                    //0
                    $t['ntype'] = L('链接');
    ...
    public function add() {
		...
		if (IS_POST) {
            ...
            if ($ntype == 0) {
                ...
                if (!$page['id']) {
                    ...
                    $ppid = $page['id'];
                    $temp = $this->models('site/page')->get($page['id']);
                    if ($temp) {
                        $data[1]['url'] = $temp['url'];
                        ...
                        if ($childs && $data[1]['extend']) {
                            $page = $this->models('site/page')->get_data_all();
                            foreach ($childs as $i) {
			...
			if (isset($data['error'])) {
				...
				$id = (int)$this->models('site/navigator')->add($data[1]);
				$this->cache(1);
                $this->system_log('添加自定义链接【'.$this->menu[$this->type].'#'.$id.'】'); // 记录日志
				$this->attachment_handle($this->uid, $this->models('site/navigator')->tablename.'-'.$id, $this->field);
				$this->admin_msg(L('操作成功，更新缓存生效'), dr_url('navigator/index', array('type' => $this->type)), 1);
			}
		} else {
            $error = '';
            $ntype = $ppid =  0;
		...
		$this->template->assign(array(
			...
			'error' => $error,
			'field' => $this->field,
			'select' => $this->_select($this->models('site/navigator')->get_data($this->type), $pid, 'class="form-control" name=\'pid\'', L('作为顶级')),
			'select_page' => $this->_select($this->models('site/page')->get_data_all(), (int)$ppid, 'class="form-control" name=\'page[id]\'', ''),
		));
    ...
    public function edit() {
		...
		$nav = $this->models('site/navigator')->get_data($this->type);
		$data = $nav[$id];
		...
		if (IS_POST) {
            ...
            if ($ntype == 0) {
                ...
                // 查询下级所有数据项
                if ($ppid && $page['extend']) {
                    $temp = $this->models('site/page')->get($ppid);
                    if ($temp) {
                        $childs = explode(',', $temp['childids']);
                        if ($childs) {
                            $page = $this->models('site/page')->get_data_all();
			...
			if (isset($post['error'])) {
				...
				$id = (int)$this->models('site/navigator')->edit($id, $post[1]);
                if ($ntype && $data['extend'] != $post[1]['extend']) {
                    $this->models('site/navigator')->update_extend($data['childs'], $post[1]['extend']);
					$post[1]['extend'] && $extends && $this->models('site/navigator')->set_extend($id, $data['mark'], $extends, $this->type);
                }
				$this->cache(1);
                $this->system_log('修改自定义链接【'.$this->menu[$this->type].'#'.$id.'】'); // 记录日志
				$this->attachment_handle($this->uid, $this->models('site/navigator')->tablename.'-'.$id, $this->field, $data);
				$this->admin_msg(L('操作成功，更新缓存生效'), dr_url('navigator/index', array('type' => $this->type)), 1);
			}
		}
		...
		$this->template->assign(array(
            ...
            'catid' => (int)$catid,
			'field' => $this->field,
			'select' => $this->_select($nav, $data['pid'], 'class="form-control" name=\'pid\'', L('作为顶级')),
            'select_page' => $this->_select($this->models('site/page')->get_data_all(), (int)$ppid, 'class="form-control" disabled name=\'page[id]\'', ''),
		));
		$this->template->display('navigator_add.html');
	}
    ...
    public function target() {
		if ($this->is_auth('admin/navigator/edit')) {
            ...
            $this->system_log('修改自定义链接【'.$this->menu[$this->type].'#'.$id.'】'); // 记录日志
			$this->cache(1);
			$this->admin_msg(L('操作成功，更新缓存生效'), dr_url('navigator/index', array('type' => $data['type'])), 1);
		} else {
    ...
    public function cache($update = 0) {
		$this->models('site/navigator')->cache(isset($_GET['site']) && $_GET['site'] ? (int)$_GET['site'] : SITE_ID);
