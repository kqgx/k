...
class Block extends M_Controller {
    ...
    public function __construct() {
		...
		$this->template->assign(array(
			'menu' => $this->get_menu_v3(array(
				L('自定义内容') => array('admin/block/index', 'file'),
				L('添加') => array('admin/block/add', 'plus'),
			)),
			'type' => array(
				'1' => L('纯文本'),
				'2' => L('富文本'),
    ...
    public function add() {
		...
		if (IS_POST) {
			...
			// 格式化内容
			$data = array(
				'name' => $post[1]['name'] ? $post[1]['name'] : L('未命名'),
                'code' => safe_replace($post[1]['code']),
			);
			if (!$data['code']) {
                $this->admin_msg(L('别名不能为空'));
			...
			switch (intval($_POST['type'])) {
				...
				case 4:
					// 多文件
					$data['content'] = '{i-4}:'.($post[1]['value_4'] ? array2string($post[1]['value_4']) : '');
					break;
			}
			...
			$this->admin_msg(L('操作成功，更新缓存生效'), dr_url('block/index'), 1);
		}
    ...
    public function cache($update = 0) {
		$this->models('site/block')->cache(isset($_GET['site']) && $_GET['site'] ? (int)$_GET['site'] : SITE_ID);
