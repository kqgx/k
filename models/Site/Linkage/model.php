...
class Site_Linkage_model extends CI_Model {
	...
	public function add($data) {
		...
		if (!$data || !$data['name']) {
            return array('error' => L('联动菜单名称不能为空'), 'name' => 'name');
        } elseif ($this->code_exitsts($data['code'])) {
            return array('error' => L('联动菜单不存在!'), 'name' => 'code');
        }
	...
	public function edit($id, $data) {
		...
		if (!$id) {
            return array('error' => L('对不起，数据被删除或者查询不存在'), 'name' => 'name');
	...
	public function adds($key, $data) {
		...
		if (!$key) {
            return array('error' => L('联动菜单不存在!'), 'name' => 'name');
        } elseif (!$data) {
            return array('error' => L('联动菜单名称不能为空'), 'name' => 'name');
        }
        ...
        if ($_POST['_all']) {
            ...
            if (!$names) {
                return array('error' => L('联动菜单名称不能为空'), 'name' => 'names');
            }
            foreach ($names as $t) {
                $t = trim($t);
                if (!$t) {
	...
	public function edits($key, $id, $data, $save = array()) {
		...
		if (!$data || !$data['name']) {
            ...
            return array('error' => L('别名已经存在,不能重复'), 'name' => 'cname');
        } elseif (is_numeric($data['cname']) && $data['cname'] != $id) {
            return array('error' => L('别名不能是数字'), 'name' => 'cname');
        }
