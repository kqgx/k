...
class Module_Search_model extends CI_Model {
    ...
    public function __construct() {
        ...
        $this->indexname = $this->db->dbprefix(SITE_ID.'_'.APP_DIR.'_search_index');
    ...
    public function module($dir, $siteid = SITE_ID) {
        ...
        $this->tablename = $this->db->dbprefix($siteid.'_'.$dir.'_search');
        $this->indexname = $this->db->dbprefix($siteid.'_'.$dir.'_search_index');
    }
    ...
    public function get($id, $page = 1) {
        ...
        $data['params'] = string2array($data['params']);
    ...
    public function set($get) {
        ...
        $order = $get['order'];
        $keyword = safe_replace($get['keyword']);
                ...
                foreach ($get as $i => $v) {
                    if (strpos($i, 'mall_') === 0) {
                        ...
                        if (isset($property[$n]) && $property[$n] && strlen($v)) {
                            $pwhere[] = '`id` IN (select `cid` from '.$this->db->dbprefix(SITE_ID.'_'.$this->dir.'_property_search').' where pid='.intval($property[$n]['id']).' and value = "'.safe_replace($v).'")';
                        }
                    }
                }
            }
    ...
    public function install() {
        ...
        $this->link->query(trim("
			CREATE TABLE IF NOT EXISTS `{$this->indexname}` (
			  `id` varchar(32) NOT NULL,
    ...
    public function uninstall() {
        ...
        $this->link->query("DROP TABLE IF EXISTS `{$this->indexname}`");
    }
}