...
class Extends_Extend_Admin extends M_Controller {
    ...
    public function __construct() {
		...
		if ($this->admin['adminid'] > 1 && $this->catrule && !$this->catrule['show']) {
			IS_AJAX && $this->msg(0, L('当前管理组角色您无权限操作此模块内容'));
			$this->admin_msg(L('当前管理组角色您无权限操作此模块内容'));
		} else {
    ...
    public function index() {
		...
		if (IS_POST && !$this->input->post('search')) {
			...
			switch($this->input->post('action')) {
		        // 删除
				case 'del':
					$this->catrule['del'] ? $this->models('module/content')->delete_extend_for_ids($ids) : $this->msg(0, L('您无权限操作'));
					$this->system_log('删除站点【#'.SITE_ID.'】模块【'.APP_DIR.'】扩展内容【#'.@implode(',', $ids).'】'); // 记录日志
                    $this->msg(1, L('操作成功，正在刷新...'));
					break;
				// 排序
				case 'order':
					!$this->catrule['edit'] && $this->msg(0, L('您无权限操作'));
					$_data = $this->input->post('data');
					foreach ($ids as $id) {
						$this->db->where('id', $id)->update($this->models('module/content')->prefix.'_extend', $_data[$id]);
					}
				...
				case 'move':
					...
					!$type && $this->msg(0, L('您无权限操作'));
					!$this->catrule['edit'] && $this->msg(0, L('您无权限操作'));
					$this->models('module/content')->extend_move($ids, $type);
		...
		// 数据库中分页查询
		list($list, $param)	= $this->models('module/content')->extend_limit_page($this->content['id'], $param);
		...
		if ($this->content) {
			$menu = array(
				L('章节管理') => array($this->_get_back_url(APP_DIR.'/extend/index'), 'table'),
				L('发布') => array(APP_DIR.'/admin/extend/add/cid/'.$this->content['id'].'/catid/'.$this->content['catid'], 'plus'),
				L('返回内容管理') => array($this->_get_back_url(APP_DIR.'/home/index'), 'reply')
			);
		} else {
			$menu = array(
				L('章节管理') => array($this->_get_back_url(APP_DIR.'/extend/index'), 'table'),
    ...
    public function add() {
		...
		!$this->catrule['add'] && $this->admin_msg(L('您无权限操作'));
		...
		if (IS_POST) {
			...
			if (isset($data['error'])) {
                ...
                if ($this->input->post('action') == 'draft') {
                    ...
                    $this->admin_msg(L('已保存到我的草稿箱中'), dr_url(APP_DIR.'/home/draft/'), 1);
                    exit;
                }
                // 数据来至草稿时更新时间
				...
				if ($id = $this->models('module/content')->add_extend($data)) {
					...
					$did && $this->models('module/content')->delete_draft($did, 'cid='.$this->content['id'].' and eid=-1') 
						? $this->attachment_replace_draft($did, $this->content['id'], $id, $this->models('module/content')->prefix)
                    	: $this->clear_cache('save_'.APP_DIR.'_extend_'.$this->uid);
					// mark
					$mark = $this->models('module/content')->prefix.'-'.$this->content['id'].'-'.$id;
					$member = $this->models('member')->get_base_member($this->content['uid']);
    ...
    public function edit() {
		...
		!$this->catrule['edit'] && $this->admin_msg(L('您无权限操作'));
		...
		if (IS_POST) {
			...
			if (isset($data['error'])) {
				...
				if ($id = $this->models('module/content')->edit_extend($_data, $data)) {
                    // 发布草稿时删除草稿数据
					$did && $this->models('module/content')->delete_draft($did, 'cid='.$this->content['id'].' and eid='.$id) && $this->attachment_replace_draft($did, $this->content['id'], $id, $this->models('module/content')->prefix);
                    $mark = $this->models('module/content')->prefix.'-'.$this->content['id'].'-'.$id;
					// 操作成功处理附件
