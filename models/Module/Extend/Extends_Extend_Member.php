...
class Extends_Extend_Member extends M_Controller {
    ...
    public function __construct() {
        ...
        $this->content = $this->models('module/content')->get($cid);
        $this->router->method != 'buy' && !$this->content && $this->member_msg(L('对不起，数据被删除或者查询不存在'));
        // 判断具有此栏目的管理权限
        $this->catrule = $this->module_rule[$this->content['catid']];
        $this->content['type'] = string2array($this->content['type']);
    ...
    public function index() {
        ...
        // 作者判断
        $this->content['uid'] != $this->uid && $this->member_msg(L('无权限操作'));
        ...
        $order = dr_get_order_string(safe_replace($this->input->get('order', TRUE)), 'displayorder desc');
    ...
    public function add() {
        ...
        // 添加权限
        !$this->catrule['add'] && $this->member_msg(L('您无权限操作'));
        ...
        if (IS_POST) {
            ...
            if (isset($data['error'])) {
                ...
                if ($this->input->post('action') == 'draft') {
                    ...
                    $id = $this->models('module/content')->save_draft($did, $data, 1);
                    $this->attachment_handle($this->uid, $this->models('module/content')->prefix.'_draft-'.$id, $this->field);
                    $this->member_msg(L('已保存到我的草稿箱中'), dr_member_url(APP_DIR.'/home/draft/'), 1);
                    exit;
                }
                if ($id = $this->models('module/content')->add_extend($data)) {
                    // 发布草稿时删除草稿数据
                    $did && $this->models('module/content')->delete_draft($did, 'cid='.$this->content['id'].' and eid=-1')
                        ? $this->attachment_replace_draft($did, $this->content['id'], $id, $this->models('module/content')->prefix, $data[1]['status'])
                        : $this->clear_cache('save_'.APP_DIR.'_extend_'.$this->uid);
                    if ($data[1]['status'] >= 9) {
                        $mark = $this->models('module/content')->prefix.'-'.$this->content['id'].'-'.$id;
                        ...
                        (IS_AJAX || IS_API_AUTH) && $this->msg(1, L('发布成功，马上返回列表'), dr_member_url(APP_DIR.'/extend/index', array('cid' => $this->content['id'])), $id);
                        $this->template->assign(array(
                        ...
                        $this->models('member')->admin_notice('content', L('%s 修改内容扩展审核', $this->get_cache('module-'.SITE_ID.'-'.APP_DIR, 'name')), APP_DIR.'/admin/extend/edit/id/'.$id);
                        $this->attachment_handle($this->uid, $this->models('module/content')->prefix.'_verify-'.$this->content['id'].'-'.$id, $this->field);
                        (IS_AJAX || IS_API_AUTH) && $this->msg(1, L('发布成功，请等待管理员审核'), dr_member_url(APP_DIR . '/everify/index'), $id);
                        $this->template->assign(array(
        ...
        $this->template->assign(array(
            ...
            'draft_list' => $this->models('module/content')->get_draft_list('cid='.$this->content['id'].' and eid=-1'),
            'result_error' => $error,
        ));
    ...
    public function edit() {
        ...
        // 修改权限
        !$this->catrule['edit'] && $this->member_msg(L('您无权限操作'));
        ...
        !$data && $this->member_msg(L('对不起，数据被删除或者查询不存在'));
        ...
        if (IS_POST) {
            ...
            if (isset($data['error'])) {
                ...
                // 正常保存
                if ($id = $this->models('module/content')->edit_extend($_data, $data)) {
                    // 发布草稿时删除草稿数据
                    $did && $this->models('module/content')->delete_draft($did, 'cid='.$this->content['id'].' and eid='.$id) && $this->attachment_replace_draft($did, $this->content['id'], $id, $this->models('module/content')->prefix, $data[1]['status']);
                    if ($data[1]['status'] >= 9) {
                        $mark = $this->models('module/content')->prefix.'-'.$this->content['id'].'-'.$id;
                        // 操作成功处理附件
						...
						if ($this->get_cache('module-'.SITE_ID.'-'.APP_DIR, 'category', $this->content['catid'], 'setting', 'html')
                            && $data[1]['status'] == 10) {
							$html = $this->db->where('rid', $id)->where('type', 2)->get($this->models('module/content')->prefix.'_html')->row_array();
							if ($html) {
        ...
        $this->template->assign(array(
            ...
            'draft_list' => $this->models('module/content')->get_draft_list('cid='.$this->content['id'].' and eid='.$id),
            'result_error' => $error,
        ));
        $this->template->display('module_extend_add.html');
    }
    ...
    public function del() {
        ...
        // 删除权限
        !$this->catrule['del'] && $this->member_msg(L('您无权限操作'));
        ...
        if ($id) {
            $data = $this->db
                         ->select('tableid,id')
                         ->where('id', $id)
                         ->get($this->models('module/content')->prefix.'_extend')
                         ->row_array();
            $data && $this->models('module/content')->delete_extend_for_id($id, $this->content['id'], $data['tableid']);
        }
        $this->msg(1, L('操作成功，正在刷新...'));
    }
	...
	public function status() {
		...
		if ($data['status'] == 10) {
			$this->db->where('id', $id)->update($this->models('module/content')->prefix.'_extend', array('status' => 9));
			$this->db->where('id', $id)->update($this->models('module/content')->prefix.'_extend_index', array('status' => 9));
            // 调用方法状态更改方法
            $data['status'] = 9;
            $this->models('module/content')->_update_status_extend($data);
			...
			$this->db->where('id', $id)->update($this->models('module/content')->prefix.'_extend', array('status' => 10));
			$this->db->where('id', $id)->update($this->models('module/content')->prefix.'_extend_index', array('status' => 10));
            // 调用方法状态更改方法
            $data['status'] = 10;
            $this->models('module/content')->_update_status_extend($data);
