...
class Extends_Content extends M_Controller {
    ...
    private function _get_content_menu() {
        ...
        return array(
            L('提取缩略图') => APP_DIR.'/admin/content/index',
            L('更新URL') => APP_DIR.'/admin/content/url',
            L('替换内容') => APP_DIR.'/admin/content/replace',
            L('提取关键字') => APP_DIR.'/admin/content/keyword',
        );
    }
    ...
    public function keyword() {
        ...
        if (IS_POST) {
            ...
            // 统计数量
            $total = $keyword ? $query->where('keywords=""')->count_all_results($this->models('module/content')->prefix) : $query->count_all_results($this->models('module/content')->prefix.'_index');
            ...
            if ($total) {
                ...
                $this->msg(L('可更新内容%s条，正在准备执行...', $total), dr_url(APP_DIR.'/content/keyword', array('todo' => 1)), 2);
            } else {
        ...
        if ($this->input->get('todo')) {
            ...
            $this->msg(L('正在执行中(%s) ... ', "$tpage/$page"), dr_url(APP_DIR.'/content/keyword', array('todo' => 1, 'page' => $page + 1)), 2, 0);
        } else {
    ...
    public function index() {
        ...
        if (IS_POST) {
            ...
            // 统计数量
            $total = $thumb ? $query->where('thumb=""')->count_all_results($this->models('module/content')->prefix) : $query->count_all_results($this->models('module/content')->prefix.'_index');
            ...
            if ($total) {
                ...
                $this->msg(L('可更新内容%s条，正在准备执行...', $total), dr_url(APP_DIR.'/content/index', array('todo' => 1)), 2);
            } else {
                $this->msg(L('抱歉，没有找到可更新的内容'));
            }
        }
        ...
        if ($this->input->get('todo')) {
            ...
            $this->msg(L('正在执行中(%s) ... ', "$tpage/$page"), dr_url(APP_DIR.'/content/index', array('todo' => 1, 'page' => $page + 1)), 2, 0);
        } else {
            $this->template->assign(array(
                'menu' => $this->get_menu($this->_get_content_menu()),
                'select' => $this->select_category($this->get_cache('module-'.SITE_ID.'-'.APP_DIR, 'category'), 0, 'id="dr_synid" name=\'catid[]\' multiple style="width:200px;height:250px;"', ''),
    ...
    public function url() {
        ...
        if (IS_POST) {
            ...
            // 统计数量
            $total = $query->count_all_results($this->models('module/content')->prefix.($extend ? '_extend' : '').'_index');
            $this->cache->file->save($cfile, array('catid' => $catid, 'total' => $total), 10000);
            if ($total) {
                $this->system_log('站点【#'.SITE_ID.'】模块【'.APP_DIR.'】更新URL地址#'.$total); // 记录日志
                $this->msg(L('可更新内容%s条，正在准备执行...', $total), dr_url(APP_DIR.'/content/url', array('todo' => 1, 'extend' => $extend)), 2);
            } else {
                $this->msg(L('抱歉，没有找到可更新的内容'));
            }
        }
        ...
        if ($this->input->get('todo')) {
            ...
            if ($cache) {
                ...
                $catid = 0;
                $total = $this->db->count_all_results($this->models('module/content')->prefix.($extend ? '_extend' : '').'_index');
            }
            $tpage = ceil($total / $psize); // 总页数
            if ($page > $tpage) {
                // 更新完成删除缓存
            ...
            $module = $this->get_cache('module-'.SITE_ID.'-'.APP_DIR);
            if ($extend) {
                $table = $this->models('module/content')->prefix.'_extend';
                $catid && $this->db->where_in('catid', $catid);
                $data = $this->db->limit($psize, $psize * ($page - 1))->order_by('id DESC')->get($table)->result_array();
                foreach ($data as $e) {
            ...
            $this->msg(L('正在执行中(%s) ... ', "$tpage/$page"), dr_url(APP_DIR.'/content/url', array('todo' => 1, 'extend' => $extend, 'page' => $page + 1)), 2, 0);
        } else {
            $this->template->assign(array(
                'menu' => $this->get_menu($this->_get_content_menu()),
    ...
    public function replace() {
        ...
        if (IS_POST) {
            ...
            $tj = str_replace([';'], '', $this->input->post('tj'));
            (!$fd || !$t1) && $this->msg('“待替换字段”和“被替换内容”必须填写！');
            $fd == 'id' && $this->msg('主键不支持替换！');
            // 表名判断
            $field = $this->get_table_field(str_replace('{id}', '0', $bm));
            !$field && $this->msg('此表【'.str_replace('{id}', '0', $bm).'】无可用字段');
            // 可用字段判断
            !isset($field[$fd]) && $this->msg('在表【'.str_replace('{id}', '0', $bm).'】中没有找到字段【'.$fd.'】');
            ...
            $this->msg('正在搜索替换...', dr_url(APP_DIR.'/content/replace', array('todo' => 1)), 2);
        }
        ...
        if ($this->input->get('todo')) {
            ...
            if (!$cache['fd'] || !$cache['t1']) {
                $this->cache->file->delete($cfile);
                $this->msg('缓存失败：“待替换字段”和“被替换内容”必须填写！');
            }
            $count = 0;
            ...
            $this->cache->file->delete($cfile);
            $this->msg('替换完毕，共替换'.$count.'条数据', NULL, 1);
        } else {
            $bm = array(
                array('name' => '主表', 'table' => $this->models('module/content')->prefix),
                array('name' => '附表', 'table' => $this->models('module/content')->prefix.'_data_{id}'),
                array('name' => '栏目主表', 'table' => $this->models('module/content')->prefix.'_category_data'),
                array('name' => '栏目附表', 'table' => $this->models('module/content')->prefix.'_category_data_{id}'),
            );
            if ($this->get_cache('module-'.SITE_ID.'-'.APP_DIR, 'extend')) {
               $bm[] = array('name' => '扩展主表', 'table' => $this->models('module/content')->prefix.'_extend');
               $bm[] = array('name' => '扩展附表', 'table' => $this->models('module/content')->prefix.'_extend_data_{id}');
            }
            $form = $this->get_cache('module-'.SITE_ID.'-'.APP_DIR, 'form');
            if ($form) {
                foreach ($form as $t) {
                    $bm[] = array('name' => $t['name'].'主表', 'table' => $this->models('module/content')->prefix.'_form_'.$t['table']);
                    $bm[] = array('name' => $t['name'].'附表', 'table' => $this->models('module/content')->prefix.'_form_'.$t['table'].'_data_{id}');
                }
            }
            $this->template->assign(array(
                'bm' => $bm,
