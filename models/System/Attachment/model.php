...
class System_Attachment_model extends CI_Model {
    ...
    public function limit($uid, $page, $pagesize, $ext, $table) {
        ...
        $sql = ' `'.$this->db->dbprefix('attachment').'` AS `a`,`'.$this->db->dbprefix('attachment_use').'` AS `b`';
    ...
    public function limit_page($uid, $ext, $total, $page) {
        ...
        $sql = 'FROM `'.$this->db->dbprefix('attachment').'` AS `a`,`'.$this->db->dbprefix('attachment_use').'` AS `b` ';
        $sql.= 'WHERE (`a`.`id`=`b`.`id` AND `a`.`siteid`='.$this->siteid.' AND `a`.`uid`='.$uid.')';
    ...
    public function delete_for_handle($uid, $related, $id) {
        ...
        $info = $this->db->select('attachment,remote')->where('id', (int)$id)->get('attachment_use')->row_array();
        if (!$info) {
            return NULL;
        }
    ...
    public function delete_for_table($related, $is_all = FALSE) {
        ...
        foreach ($data as $t) {
            ...
            $info = $this->db->select('attachment,remote')->where('id', $t['id'])->get('attachment_use')->row_array();
            if (!$info) {
                return NULL;
            }
    ...
    public function get_my_attach($uid, $ext, $limit = 20) {
        ...
        $this->db->order_by('inputtime DESC');
        $tableid = (int)substr((string)$uid, -1, 1);
        $data = $this->db->get('attachment_use')->result_array();
    ...
    public function catcher($uid, $url) {
        ...
        if (!$this->domain) {
            ...
            // 域名验证
            $this->domain = require CONFPATH.'domain.php';
    ...
    public function cron_catcher($uid, $url, $field) {
        ...
        $this->db->insert('cron_queue', array(
            'type' => 6,
            'value' => array2string(array('id'=>$id, 'uid'=>$uid, 'url'=>$url, 'field' => $field)),
            'error' => '',
            'status' => 0,
            'inputtime' => SYS_TIME,
            'updatetime' => 0,
    ...
    public function upload($uid, $info, $id = 0) {
        ...
        if ($attachinfo['width']) {
            ...
            require_once LIBRARIES.'Wp_image.php';
            // 获取系统配置尺寸
        ...
        if ($replace) {
            ...
            // 更新替换已使用的附件表
            $this->db->where('id', $id)->update('attachment_use', array(
                'uid' => $uid,
                'author' => $author,
                'remote' => $remote,
                'fileext' => $_ext,
            ...
            $this->db->replace('attachment_unused', array(
                ...
                'inputtime' => SYS_TIME,
                'attachment' => $attachment,
                'attachinfo' => array2string($attachinfo),
            ));
        }
    ...
    public function upload2($id, $config, $file, $info = array(), $thumb = array(), $filesize = 0) {
        ...
        if ($info && isset($info['id']) && $info['id'] && $attachment) {
            $this->db->where('id', (int)$info['id'])->update('attachment_use', array(
                'remote' => $remote,
                'attachment' => $attachment,
            ));
    ...
    public function _delete_attachment($info) {
        ...
        if (!isset($info['attachment'])) {
            // 重新查询
            $info = $this->db->where('id', (int)$info['id'])->get('attachment_use')->row_array();
        }
        ...
        isset($info['tableid']) && $this->db->delete('attachment_use', 'id='.(int)$info['id']);
    ...
    public function do_cron($id, $param, $block = 0) {
        ...
        // 生成授权文件
        file_put_contents(WEBPATH.'cache/file/'.md5($id).'.auth', array2string(array(
            'time' => SYS_TIME,
            'param' => $param,
        )));
