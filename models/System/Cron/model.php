...
class System_Cron_model extends CI_Model {
    ...
    public function _execute_sinashare($data) {
        ...
        require_once FCPATH.'libraries/Share/Sina.php';
    ...
    public function _execute_sms($data) {
        ...
        $file = CONFPATH.'sms.php';
        if (!is_file($file)) {
            return '你都还没有配置短信账号呢';
        }

        $result = $this->models('system/sms')->send($data['mobile'], $data['content']);
        if ($result === FALSE) {
            return '短信发送失败';
        } else {
    ...
    public function _execute_down_file($data) {
        ...
        if (@file_put_contents($path.$filename.'.'.$fileext, $file)) {
            ...
            $result = $this->models('system/attachment')->upload($data['uid'], $info, $data['id']);
            if (is_array($result)) {
                if (!$result['related']) {
                ...
                if ($data['field']['relatedname'] == 'module') {
                    ...
                    if (strpos($table, '_draft') || strpos($table, '_verify')) {
                        ...
                        $content = string2array($row['content']);
