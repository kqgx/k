...
class Admin_Auth_model extends CI_Model{
	...
	public function get_role($id) {
		...
		$data['system'] = string2array($data['system']);
		$data['module'] = string2array($data['module']);
		$data['application'] = string2array($data['application']);
	...
	public function get_auth_all() {
		...
		if ($module) {
			foreach ($module as $t) {
				if (is_file(WEBPATH.'module/'.$t['dirname'].'/config/auth.php')) {
					$config = array();
					require WEBPATH.'module/'.$t['dirname'].'/config/auth.php';
                    foreach ($config['auth'] as $i => $c) {
                        if (isset($c['auth']['admin/tpl/index'])) {
                            // 增加移动端模板权限
                            $config['auth'][$i]['auth']['admin/tpl/mobile'] = L('移动端模板');
                            break;
                        }
                    }
                ...
                if ($mform[$t['dirname']]) {
                    foreach ($mform[$t['dirname']] as $tt) {
                        $data = array_merge($data, array(
                            $t['dirname'].'/admin/form_'.$tt['table'].'/index' => L($tt['name']),
                            $t['dirname'].'/admin/form_'.$tt['table'].'/add' => L('添加'),
                            $t['dirname'].'/admin/form_'.$tt['table'].'/edit' => L('修改'),
                            $t['dirname'].'/admin/form_'.$tt['table'].'/del' => L('删除'),
                            $t['dirname'].'/admin/form_'.$tt['table'].'/show' => L('查看'),
                        ));
                    }
                }
			}
        ...
        if ($form) {
            foreach ($form as $t) {
                $data = array_merge($data, array(
                    'admin/form_'.$t['table'].'/index' => L($t['name'].'管理'),
	...
	public function cache() {
		...
		foreach ($data as $t) {
			...
			$t['system'] = string2array($t['system']);
			$t['module'] = string2array($t['module']);
			$t['application'] = string2array($t['application']);
			$cache[$t['id']] = $t;
		}
